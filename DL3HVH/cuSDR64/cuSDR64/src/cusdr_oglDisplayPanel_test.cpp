...
void OGLDisplayPanel::paintUpperRegion() {
	...
	if (m_settings->getHermesPresence()) {
...
void OGLDisplayPanel::paintSMeter() {
	...
	if (!m_smeterFBO || m_smeterUpdate || m_smeterRenew) {
		...
		if (!m_smeterFBO || m_smeterRenew) {
			...
			//m_smeterFBO = new QGLFramebufferObject(width, height);
			m_smeterFBO = new QGLFramebufferObject(m_sMeterWidth, height);//, format);
	...
	// draw background
	/*if (m_dataEngineState == QSDR::DataEngineUp)
		drawGLRect(m_smeterRect, Qt::black, m_bkgColor2, -3.0f, false);
	else
		drawGLRect(m_smeterRect, Qt::black, -3.0f);*/

	QRect rect = QRect(m_rxRect.right() + m_sMeterOffset, 0, m_sMeterWidth, height);
	//renderTexture(m_smeterRect, m_smeterFBO->texture(), -1.0f);
	...
	if (m_dataEngineState == QSDR::DataEngineUp) {
		...
		if (m_SMeterA) {
			...
			if (m_sMeterMinValueA < -156*ONEPI/256.0f)
				t = -156*ONEPI/256.0f;
			else
				t = m_sMeterMinValueA;

			glLineWidth(3);
			qglColor(QColor(250, 90, 55));
		
			glBegin(GL_LINE_STRIP);
			for (int i = 0; i < m_smeterVertices; ++i) {
		
				glVertex3f(X + R * qCos(t), Y + R * qSin(t), 1.0);
				t += TWOPI / m_smeterVertices;
				if (t > m_sMeterMaxValueA) break;
			}
			glEnd();

			//qglColor(QColor(215, 215, 215));
			//m_oglTextBig->renderText(X - mid + 30, Y - R0 - 40, m_sMeterNumValueString);
			...
			glBegin(GL_LINES);
			glVertex3f(X, Y, 1.0);
			glVertex3f(X + R1 * qCos(m_sMeterValue), Y + R1 * qSin(m_sMeterValue), 2.0);
			glEnd();
		}
		else {
			...
			int min = (int)m_sMeterMinValueB;
			int max = (int)m_sMeterMaxValueB;
			min += min%2;
			max += max%2;
			...
			if (min > 0) {
				...
				for (int i = min; i <= max; i += 2) {
					...
					glBegin(GL_LINES);
						...
						glVertex3i(m_rxRect.right() + m_sMeterOffset + i, m_sMeterPosY + 3, 0.0);
						glVertex3i(m_rxRect.right() + m_sMeterOffset + i, m_sMeterPosY + 10, 0.0);
					glEnd();
				}
			}
			...
			if (m_sMeterValue > 0) {
				...
				glBegin(GL_LINES);
					/*glVertex3f(x1 + (int)m_sMeterValue, m_sMeterPosY - 4, 1.0);
					glVertex3f(x1 + (int)m_sMeterValue, m_sMeterPosY + 18, 1.0);*/
					glVertex3f(m_rxRect.right() + m_sMeterOffset + (int)m_sMeterValue, m_sMeterPosY - 4, 1.0);
					glVertex3f(m_rxRect.right() + m_sMeterOffset + (int)m_sMeterValue, m_sMeterPosY + 18, 1.0);
				glEnd();
			}
			...
			/*m_oglTextBig->renderText(x1 + 2, 80, 3.0, m_sMeterNumValueString);
			m_oglTextNormal->renderText(x1 + 56, 86, 3.0, "dBm");*/
			m_oglTextBig->renderText(x1 + width - 85, 80, 3.0, m_sMeterNumValueString);
			m_oglTextNormal->renderText(x1 + width - 28, 86, 3.0, "dBm");
...
void OGLDisplayPanel::renderSMeterB() {
	...
	if (m_dataEngineState == QSDR::DataEngineUp)
		...
		drawGLRect(QRect(0, 0, x2-x1, y2-y1), Qt::black);
		//drawGLRect(m_smeterRect, Qt::black);
	...
	for (int x = markerSpacing - 16, y = x + markerSpacing/2, z = -130; z < 10; x += markerSpacing, y += markerSpacing, z += 10) {
		...
		if (z == -120 || z == -100) 
			m_oglTextNormal->renderText(x-13, m_sMeterPosY - 18, marker);

		/*if (z == -100) 
			m_oglTextNormal->renderText(x-13, m_sMeterPosY - 18, marker);*/
		
		if (z == -80 || z == -60 || z == -40 || z == -20)
			m_oglTextNormal->renderText(x-10, m_sMeterPosY - 18, marker);
		
		if (z == 0)
			m_oglTextNormal->renderText(x-4, m_sMeterPosY - 18, marker);
	}

	//m_oglTextNormal->renderText(width - 25, m_sMeterPosY - 18, "dBm");
...
void OGLDisplayPanel::setupDisplayRegions(QSize size) {
	...
	//DISPLAYPANEL_DEBUG << "width:" << width;
	//DISPLAYPANEL_DEBUG << "m_rxRect width:" << width - m_sMeterWidth;

	//m_rxRect = QRect(0, 0, width - m_sMeterWidth, height);
	//m_smeterRect = QRect(m_rxRect.right(), 0, m_sMeterWidth, height);

	m_rxRect = QRect(0, 0, m_rxRectWidth, height);
	
	m_sMeterWidth = (int)(width - m_rxRectWidth);
	...
	if (width - m_rxRectWidth < m_sMeterWidth) {
	...
	//DISPLAYPANEL_DEBUG << "m_smeterRect:" << m_smeterRect;
	...
	m_freg1 		 = QRegion(QRect(162, 35, 8, fontHeightFrq2));
	m_freg10 		 = QRegion(QRect(148, 35, 8, fontHeightFrq2));
	m_freg100 		 = QRegion(QRect(134, 35, 8, fontHeightFrq2));
	m_freg1000 		 = QRegion(QRect(112, 35, 12, fontHeightFrq1));
	m_freg10000 	 = QRegion(QRect(92, 35, 12, fontHeightFrq1));
	m_freg100000	 = QRegion(QRect(72, 35, 12, fontHeightFrq1));
...
void OGLDisplayPanel::setSMeterValue(int rx, float value) {
	...
	if (m_SMeterA) {
		...
		float tmp = (1.9894f * value + 189.017f);
