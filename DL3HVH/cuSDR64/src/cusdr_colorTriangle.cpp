/**
* @file cusdr_colorTriangle.cpp
* @brief Color triangle class for cuSDR
*/
...
void QtColorTriangle::polish()
{
    ...
    penWidth = (int) qFloor(outerRadius / 50.0);
    ellipseSize = (int) qFloor(outerRadius / 12.5);

    double cx = (double) contentsRect().center().x();
    double cy = (double) contentsRect().center().y();

    pa = QPointF(cx + (qCos(a) * (outerRadius - (outerRadius / 5.0))),
		     cy - (qSin(a) * (outerRadius - (outerRadius / 5.0))));
    pb = QPointF(cx + (qCos(b) * (outerRadius - (outerRadius / 5.0))),
		     cy - (qSin(b) * (outerRadius - (outerRadius / 5.0))));
    pc = QPointF(cx + (qCos(c) * (outerRadius - (outerRadius / 5.0))),
		     cy - (qSin(c) * (outerRadius - (outerRadius / 5.0))));
    pd = QPointF(cx + (qCos(a) * (outerRadius - (outerRadius / 10.0))),
		     cy - (qSin(a) * (outerRadius - (outerRadius / 10.0))));
...
void QtColorTriangle::genBackground()
{
	...
	//p.fillRect(bg.rect(), QColor::fromRgb(190,206,188));
	p.fillRect(bg.rect(), QColor::fromRgb(10,10,10));

	//p.setPen(QColor::fromRgb(59,90,74));
...
{
    outerRadius = (contentsRect().width() - 10) / 2;
    if ((contentsRect().height() - 1) / 2 < outerRadius)
	outerRadius = (contentsRect().height() - 10) / 2;
