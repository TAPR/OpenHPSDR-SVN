...
ChirpProcessor::ChirpProcessor(THPSDRParameter *ioData)
	...
	, m_filterLowerFrequency((float)m_settings->getChirpFilterLowerFrequency())
	, m_filterUpperFrequency((float)m_settings->getChirpFilterUpperFrequency())
{
	...
	//memset(m_cpxIn, 0, FULL_BUFFERSIZE * sizeof(CPX));
...
void ChirpProcessor::setSpectras(const float *distance, const float *chirpfft) {
	...
	if (m_showChirpFFT)
		m_settings->setChirpSpectrumBuffer(m_sampleRate/m_downRate, FULL_BUFFERSIZE, chirpfft);
		...
		if (m_specAvgLength > 1)
			...
			//m_settings->setChirpSpectrumBuffer(m_sampleRate/m_downRate, FULL_BUFFERSIZE, distance);
			m_settings->setChirpSpectrumBuffer(m_sampleRate/m_downRate, FULL_BUFFERSIZE/2, distance);
	}		
}
...
void ChirpProcessor::spectrumAveraging(qint64 length, const float *buffer) {
	...
	if (average_queue.size() <= m_specAvgLength) {
		...
		m_settings->setChirpSpectrumBuffer(m_sampleRate, FULL_BUFFERSIZE, m_outBuf);
		return;
	}
	...
	//m_settings->setChirpSpectrumBuffer(m_sampleRate, FULL_BUFFERSIZE, m_outBuf);
	m_settings->setChirpSpectrumBuffer(m_sampleRate, FULL_BUFFERSIZE/2, m_outBuf);
	average_queue.dequeue();
...
void ChirpProcessor::generateLocalChirp() {
	...
	qreal time =  m_settings->getChirpBufferDurationUs() / 1.0E6;
	qint64 length = (qint64)(sampleRate * time);
	
	qreal a = ONEPI * (m_settings->getUpperChirpFreq() - m_settings->getLowerChirpFreq()) / time;
	qreal b = TWOPI * m_settings->getLowerChirpFreq();
	...
	float dur = m_settings->getChirpBufferDurationUs() / 1000.0f;
	...
	CHIRP_PROCESSOR_DEBUG	<< "  start frequency (Hz)" << m_settings->getLowerChirpFreq();
	CHIRP_PROCESSOR_DEBUG	<< "  end frequency (Hz)" << m_settings->getUpperChirpFreq();
...
void ChirpProcessor::decimate(CPX *in, CPX *out, int size, int downrate) {
	...
	//memset(m_tmpDec, 0, size * sizeof(CPX));
	//memcpy(m_tmpDec, in, size * sizeof(CPX));
