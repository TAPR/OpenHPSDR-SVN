...
/* 	This program interfaces the LT2208 to a PC over USB.
	...
	Change log:
	 ...
	 3 Oct 2012 - Changes by Joe K5SO:
						- Reduced the number of receivers implemented to two
						- Added dual Rx, independent, manual Alex filter controls
						- Added Alex HPF/LPF switching logic to select LPF appropriate for highest Rx freq in use and
						  HPF appropriate for lowest Rx freq in use when operating with multiple receivers on a single Mercury board
	 ...
	 4 May 2013		- Removed NWire comm modules for Atlas bus lines A2 and A6, replaced i2c module 
						  with new i2c_interface and i2c_slave modules, moved transfer of firmware version number 
						  and ADC overload status to the i2c bus, implemented polyphase-fir-filter receiver modules, 
						  increased the number of receivers implemented to 3, added 384ksps sampling rate option,
						  in order to facilitate 384ksps sampling the I and Q data paths are sent to Metis/Ozy on independent
						  Atlas bus lines (specifically, Rx1_I on Atlas A10, Rx1_Q on A5, Rx2_I on A9, Rx2_Q on A4, 
						  Rx3_I on A8, Rx3_Q on A3, all run at a 384k bus line data rate regardless of the receiver sampling 
						  rate the user selects), replaced the mercury.sdc file with a new mercury.sdc timing constraint file, 
						  all clocks are constrained, timing requirements are met 100% according to TimeQuest timing wizard reports, 
						  changed the version number to v3.4.
	 
*/
	
module Mercury(OSC_10MHZ, ext_10MHZ,AUX_CLK, C122_clk,INA,CC,ATTRLY,A12,C21,C23,C24,MDOUT_I,MDOUT_Q,CDIN,
...
output wire  [2:0] MDOUT_I;        // I data out to Atlas bus on A8, A9, A10  
output wire  [2:0] MDOUT_Q;        // Q data out to Atlas bus on A3, A4, A5  
...
localparam SERIAL = 8'd34;  // software version serial number
...
assign latch[1] = (multiRx_mode > 3'b000) ? 1'b1 : 1'b0; 	//enable second receiver
assign latch[2] = (multiRx_mode > 3'b001) ? 1'b1 : 1'b0; 	//enable third receiver
...
assign C122_clk_ONOFF[1] = (gate[1]) ? C122_clk : 1'b0; 	//enable second receiver	  /assign C122_clk_ONOFF[2] = (gate[2]) ? C122_clk : 1'b0; 	//enable third receiver	  
...
generate
  genvar c;
  for (c = 0; c < NR; c = c + 1) // NR Mercury Data Channels/Receivers 
   begin: MDC 
...
*/      
    // MDO_I[c] and MDO_Q[c] are driven onto the appropriate Atlas lines according to Merc_ID for multiple-Mercury-board mode or on 
    // eight Atlas lines for multi-RX-on-a-single-Merc mode
	 
    assign MDOUT_I[c] = MULTIPLE_MERC ? ((Merc_ID == c) ? (common_Merc_freq ? MDO_I[0] : MDO_I[c]) : 1'bz) : MDO_I[c]; //1 channel or 2 channels from this Mercury
    assign MDIN_I[c*24 +: 24] = rx_I[c];
    assign MDOUT_Q[c] = MULTIPLE_MERC ? ((Merc_ID == c) ? (common_Merc_freq ? MDO_Q[0] : MDO_Q[c]) : 1'bz) : MDO_Q[c]; //1 channel or 2 channels from this Mercury
    assign MDIN_Q[c*24 +: 24] = rx_Q[c];
  end
endgenerate
   
// send I and Q data to MDO_I and MDO_Q then to MDOUT_I and MDOUT_Q via the assigns above
...
reg           PREAMP;         // "pre-amp" selection for on-board input-attenuator on Mercury (0="preamp" off, 1="preamp" ON)
reg           common_Merc_freq;
wire           common_select;
wire    [3:0] fn;

assign fn = C122_rcv_data[87:84]; // which Mercury frequency 1 - 4 
...
//      else if (C122_rcv_rdy && (fn == (j+1)))
		...
		if (C122_rcv_rdy && (common_select  | (!MULTIPLE_MERC & (fn == j+1)))) //...if common-Merc_freq set, all boards update with Merc1      
...
begin
  ...
  begin
    ...
    C122_DFS0     <= C122_rcv_data[89];    // I/Q sampling rate selection
    C122_PTT_out  <= C122_rcv_data[88];    // PTT from PC via USB 
    multiRx_mode  <= C122_rcv_data[7:5];   // 0=single receiver, 1=two receivers, 2=three receivers, 3=four receivers
    RxN_preamps   <= C122_rcv_data[4:1];	 // decode preamp settings for all Mercury boards
										// Note: presently there is a redundancy for Mercury #1 "pre-amp" selection because IF_PREAMP below also 
    ...
    if (C122_rcv_data[87:84] == 4'b0)   //all boards update with Merc1
    begin
      clock_select      <= C122_rcv_data[51:49];     
      PREAMP            <= C122_rcv_data[40];     // PREAMP = 0 => 20 dB attenuator is inserted; PREAMP = 1 => no attenuator (so-called "preamp ON") 
      DITHER            <= C122_rcv_data[39];     // 1 = dither on
      RAND              <= C122_rcv_data[38];     // 1 = randomizer on 
      C122_ATTEN        <= C122_rcv_data[37:36];  // Attenuator setting on Alex
      C122_TX_relay     <= C122_rcv_data[35:34];  // Tx relay selection on Alex
...
assign source_122MHZ = (Merc_ID > 0) ? 1'b0 : (clock_select[2] == 1'b1); // if set use internally and send to LVDS else get from LVDS
