...
namespace PowerSDR
{
	public class Common
	{
		...
		public static void ForceFormOnScreen(Form f)
		{
			...
			{
				if(f.Left > screens[i].Bounds.Left && // left bound
					f.Left < screens[i].Bounds.Right && // right bound
					f.Top > screens[i].Bounds.Top && // top bound
					f.Top < screens[i].Bounds.Bottom) // bottom bound
				{
					on_screen = true;
					break;
				}
