...
namespace PowerSDR
{
	unsafe public class FLEX5000ProdTestForm : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.udLevel.Increment = new System.Decimal(new int[] {
																	  1,
																	  0,
																	  0,
																	  65536});
			this.udLevel.Location = new System.Drawing.Point(120, 64);
			this.udLevel.Minimum = new System.Decimal(new int[] {
																	100,
																	0,
																	0,
																	-2147483648});
			...
			this.udLevel.Value = new System.Decimal(new int[] {
																  240,
																  0,
																  0,
																  -2147418112});
		...
		private void TestGenBal()
		{
			...
			if(adc_r - off_l < 55.0f)
			{
				...
				test_genbal += "ADC_R S/N <55dB ("+(adc_r-off_l).ToString("f1")+")\n";
				b = false;
			}
		...
		private void TestATTN()
		{
			...
			bool b = (Math.Abs(-20.0f - (a[peak_bin] - peak)) <= 1.0f); // pass from 19 - 21dB
			if(!b)
			{
				btnGenATTN.BackColor = Color.Red;
				test_attn = " ATTN Test: Failed delta > -20 +/-1dB ("+(a[peak_bin]-peak).ToString("f1")+")";
			}
			else
			{
				btnGenPreamp.BackColor = Color.Green;
				test_preamp = "ATTN Test: Passed ("+(a[peak_bin]-peak).ToString("f1")+")";
			}
			...
			for(int i=0; i<band_freqs.Length; i++)
			{
				...
				if(do_band)
				{
					...
					if((no_filter < -10.0f || with_filter < -10.0f) ||
						((no_filter-with_filter) > avg[i]+1.0f) ||
		...
		private void TestTXGain()
		{
			...
			path += "\\TX Gain";
			if(!Directory.Exists(path)) Directory.CreateDirectory(path);
			writer = new StreamWriter(path+"\\tx_gain_"+FWCEEPROM.SerialToString(FWCEEPROM.TRXSerial)+".csv");
