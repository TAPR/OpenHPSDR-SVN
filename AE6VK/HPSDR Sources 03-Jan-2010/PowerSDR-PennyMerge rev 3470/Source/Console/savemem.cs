...
namespace PowerSDR
{
	public class SaveMem : System.Windows.Forms.Form
	{
		...
		public SaveMem(Console c)
		{
			...
			comboMode.SelectedIndex = (int)console.CurrentDSPMode;
		...
		{
			if(comboMode.SelectedIndex < 0) return;

			int filter_index = comboFilter.SelectedIndex;
			comboFilter.Items.Clear();

			for(Filter f = Filter.FIRST+1; f<Filter.NONE; f++)
				comboFilter.Items.Add(console.filter_presets[comboMode.SelectedIndex].GetName(f));
			...
			{
				if(filter_index > 0) comboFilter.SelectedIndex = filter_index;
				else comboFilter.SelectedIndex = 0;
