	...
	public class FWCTestForm : System.Windows.Forms.Form
	{
		...
		private void PollADC()
		{
			while(!closing)
			{
				...
				if(FWC.ReadPAADC(4, out val) != 0) break;
				volts = (float)val/4096*2.5f;
				double temp_c = 305-volts*1000/2.2;
				if(temp_c >= 50 && temp_c < 70) txtTemp.BackColor = Color.Yellow;
				else if(temp_c >= 70 && temp_c < 80) txtTemp.BackColor = Color.Orange;
				else if(temp_c >= 80) txtTemp.BackColor = Color.Red;
