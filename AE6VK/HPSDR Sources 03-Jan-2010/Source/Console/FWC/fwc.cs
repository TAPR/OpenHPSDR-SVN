...
//#endif
		...
		public enum Opcode
		{
			...
			RDAL_OP_SET_CURRENT_DSP_MODE = 1306,
...
//*****************************************************************************************
		...
		public static int SetRX1Freq(float freq)
		{
			...
			if(SendParam(&freq, 4))
			{
				Debug.WriteLine("SetRX1Freq: SendParam error");
				return -1;
			}

			uint error;
			if(IssueOpcode(out error, (uint)Opcode.RDAL_OP_SET_RX1_FREQ, 1000))
			{
				Debug.WriteLine("SetRX1Freq: IssueOpcode error: "+error);
				return -2;
			}
#else
		...
		public static int SetTXFreq(float freq)
		{
			...
			if(SendParam(&freq, 4))
			{
				Debug.WriteLine("SetTXFreq: SendParam error");
				return -1;
			}

			uint error;
			if(IssueOpcode(out error, (uint)Opcode.RDAL_OP_SET_TX_FREQ, 1000))
			{
				Debug.WriteLine("SetTXFreq: IssueOpcode error: "+error);
				return -2;
			}
#else
		...
		public static int SetPreamp(bool b)
		{
#if(DAL)
			uint param = 0;
			if(b) param = 1;
...
#else
			FWCMidi.SendSetMessage(Opcode.RDAL_OP_SET_PREAMP, Convert.ToUInt32(b), 0);
#endif
			return 0;
		}
		...
		{
			FWCMidi.SendSetMessage(Opcode.RDAL_OP_FLEXWIRE_WRITE_2_VALUE, addr, (v1<<8)+v2);
			return 0;
		}

		public static int SetCurrentDSPMode(DSPMode mode)
		{
			FWCMidi.SendSetMessage(Opcode.RDAL_OP_SET_CURRENT_DSP_MODE, (uint)mode, 0);
			return 0;
		}
