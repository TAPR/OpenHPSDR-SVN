...
namespace PowerSDR
{
	unsafe public class FLEX5000ProdTestForm : System.Windows.Forms.Form
	{
		...
		public FLEX5000ProdTestForm(Console c)
		{
			...
			if(console.SetupForm.DSPRXBufferSize < 4096)
		...
		private void btnPLL_Click(object sender, System.EventArgs e)
		{
			...
			if(!file_exists) writer.WriteLine("Serial Num, Date/Time, Version, Locked");
			writer.WriteLine(FWCEEPROM.SerialToString(FWCEEPROM.TRXSerial)+","
		...
		private void TestGenBal()
		{
			...
			DSPMode dsp = console.CurrentDSPMode;
			console.CurrentDSPMode = DSPMode.DSB;

			PreampMode preamp = console.CurrentPreampMode;
			console.CurrentPreampMode = PreampMode.OFF;

			Filter filter = console.CurrentFilter;
			int var_low = console.FilterLowValue;
			int var_high = console.FilterHighValue;
			console.CurrentFilter = Filter.VAR1;
			console.FilterLowValue = -1000;
			console.FilterHighValue = 1000;
			...
			bool b = true;
			if(adc_l - off_l < 55.0f)
			{
				lstDebug.Items.Insert(0, " Gen/Bal Test: Failed ADC_L S/N <55dB ("+(adc_l-off_l).ToString("f1")+")");
				test_genbal += "Failed ADC_L S/N <55dB ("+(adc_l-off_l).ToString("f1")+")\n";
				b = false;
			}
			if(adc_r - off_l < 55.0f)
			{
				lstDebug.Items.Insert(0, " Gen/Bal Test: Failed ADC_R S/N <55dB ("+(adc_r-off_l).ToString("f1")+")");
				test_genbal += "ADC_L S/N <55dB ("+(adc_l-off_l).ToString("f1")+")\n";
				b = false;
			}
			...
			if(Math.Abs(-26.6 - adc_l) > 1.0f)
			{
				lstDebug.Items.Insert(0, " Gen/Bal Test: Failed ADC_L -26.6+/-1.0 ("+adc_l.ToString("f1")+")");
				test_genbal += "ADC_L -26.6+/-1.0 ("+adc_l.ToString("f1")+")\n";
				b = false;
			}
			if(Math.Abs(-26.1 - adc_r) > 1.0f)
			{
				lstDebug.Items.Insert(0, " Gen/Bal Test: Failed ADC_R -26.1+/-1.0 ("+adc_l.ToString("f1")+")");
				test_genbal += "ADC_R -26.1+/-1.0 ("+adc_l.ToString("f1")+")\n";
				b = false;
			}
			...
			console.CurrentDSPMode = dsp;
			console.CurrentPreampMode = preamp;
			console.CurrentFilter = filter;
			if(filter == Filter.VAR1 || filter == Filter.VAR2)
			{
				console.FilterLowValue = var_low;
				console.FilterHighValue = var_high;
			}
		...
		private void TestNoise()
		{
			...
			PreampMode preamp = console.CurrentPreampMode;
			console.CurrentPreampMode = PreampMode.HIGH;
		...
		private void TestImpulse()
		{
			...
			bool file_exists = File.Exists(path);
			StreamWriter writer = new StreamWriter(path, true);
			if(!file_exists) writer.WriteLine("Serial Num, Date/Time, Version, Impulse, Noise, Diff, Passed");
			writer.WriteLine(FWCEEPROM.SerialToString(FWCEEPROM.TRXSerial)+","
				+DateTime.Now.ToShortDateString()+" "+DateTime.Now.ToShortTimeString()+","
				+console.Text+","
		...
		private void TestPreamp()
		{
			...
			bool b = (Math.Abs(14.0 - (a[peak_bin] - peak)) <= 1.0f); // pass from 13 - 15dB
			if(!b)
			{
				btnGenPreamp.BackColor = Color.Red;
				test_preamp = " Preamp Test: Failed delta > 14 +/-1dB ("+(a[peak_bin]-peak).ToString("f1")+")";
			}
			else
			{
			...
			if(!file_exists) writer.WriteLine("Serial Num, Date/Time, Version, High, Off, Diff, Passed");
			writer.WriteLine(FWCEEPROM.SerialToString(FWCEEPROM.TRXSerial)+","
				+DateTime.Now.ToShortDateString()+" "+DateTime.Now.ToShortTimeString()+","
				+console.Text+","
		...
		private void TestRXFilter()
		{
			...
			DSPMode dsp_mode = console.CurrentDSPMode;
			console.CurrentDSPMode = DSPMode.DSB;

			Filter filter = console.CurrentFilter;
			console.CurrentFilter = Filter.VAR1;

			int filt_high = console.FilterHighValue;
			console.FilterHighValue = 500;

			int filt_low = console.FilterLowValue;			
			console.FilterLowValue = -500;
			...
			for(int i=0; i<band_freqs.Length; i++)
			{
				...
				if(do_band)
				{
					...
					if(Math.Abs((no_filter-with_filter) - avg[i]) > tol)
					{
			...
			Debug.WriteLine("");
				...
				if(test_rx_filter.StartsWith("RX Filter Test: Failed ("))
					test_rx_filter = test_rx_filter.Substring(0, test_rx_filter.Length-2)+")";
			...
			path += "\\RX Filter";
			if(!Directory.Exists(path)) Directory.CreateDirectory(path);
			writer = new StreamWriter(path+"\\rx_filter_"+FWCEEPROM.SerialToString(FWCEEPROM.TRXSerial)+".csv");
			...
			FWC.SetSig(false);
			Thread.Sleep(50);
			FWC.SetFullDuplex(full_duplex);
			Thread.Sleep(50);
			console.DisplayModeText = display;
			console.CurrentDSPMode = dsp_mode;
			console.FilterHighValue = filt_high;
			console.FilterLowValue = filt_low;
			console.CurrentFilter = filter;
			console.VFOAFreq = vfoa;
			console.VFOBFreq = vfob;
			console.CurrentPreampMode = preamp;
			console.RITOn = rit_on;
			console.SetupForm.DSPRXBufferSize = dsp_buf_size;
		...
		public void RunCalFWCRXLevel()
		{
			...
			float offset_tol = 2.0f;	// maximum distance from some average value
			float preamp_tol = 1.0f;
			...
			for(int i=0; i<band_freqs.Length; i++)
			{
				...
				if(do_band)
				{
					...
					console.VFOAFreq = band_freqs[i];
					console.VFOBFreq = band_freqs[i];
					console.CalibrateLevel(-25.0f, band_freqs[i], p);
					if(p.Text == "") break;

					if(Math.Abs(console.GetRXLevel(bands[i], 0)-offset_avg[i]) > offset_tol ||
						Math.Abs(console.GetRXLevel(bands[i], 1)-preamp_avg[i]) > preamp_tol)
					{
						...
						lstDebug.Items.Insert(0, "RX Level Test - "+BandToString(bands[i])+": Failed ("
							+console.GetRXLevel(bands[i], 0).ToString("f1")+", "
							+console.GetRXLevel(bands[i], 1).ToString("f1")+", "
							+console.GetRXLevel(bands[i], 2).ToString("f1")+")");
					}
					else
					{
			...
			for(int i=0; i<bands.Length; i++)
			{
				for(int j=0; j<3; j++)
				{
					writer.Write(console.GetRXLevel(bands[i], j).ToString("f1"));
			...
			{
				writer.Write(BandToString(bands[i])+",");
				writer.Write(console.GetRXLevel(bands[i], 0).ToString("f1")+",");
				writer.Write(console.GetRXLevel(bands[i], 1).ToString("f1")+",");
				writer.WriteLine(console.GetRXLevel(bands[i], 2).ToString("f1"));
			}
			writer.Close();

			FWCEEPROM.WriteRXLevel(console.rx_level_table);
			console.FLEX5000SyncDateTime();
		...
		public void RunCalFWCRXImage()
		{
			...
			console.VFOBFreq = vfob;
			if(test_rx_image.StartsWith("RX Image Test: Failed ("))
				test_rx_image = test_rx_image.Substring(0, test_rx_image.Length-2)+")";
			...
			for(int i=0; i<bands.Length; i++)
			{
				writer.Write(console.rx_image_gain_table[(int)bands[i]].ToString("f3")+",");
				writer.Write(console.rx_image_phase_table[(int)bands[i]].ToString("f3")+",");
			...
			path += "\\RX Image";
			if(!Directory.Exists(path))	Directory.CreateDirectory(path);
			writer = new StreamWriter(path+"\\rx_image_"+FWCEEPROM.SerialToString(FWCEEPROM.TRXSerial)+".csv");
			...
			FWCEEPROM.WriteRXImage(console.rx_image_gain_table, console.rx_image_phase_table);
			console.FLEX5000SyncDateTime();
		...
		private float CheckRXMDS(float freq, Progress progress)
		{
			...
			int var_high = console.FilterHighValue;
			console.CurrentFilter = Filter.VAR1;
			console.FilterLowValue = 50;
			console.FilterHighValue = 550;
			...
			avg = avg + console.multimeter_cal_offset
		...
		private void TestTXFilter()
		{
			...
			int filt_high = console.FilterHighValue;
			console.FilterHighValue = 700;	

			int filt_low = console.FilterLowValue;			
			console.FilterLowValue = 500;
			
			console.UpdateFilters(500, 700);
			...
			for(int i=0; i<band_freqs.Length; i++)
			{
				...
				if(do_band)
				{
					...
					if((no_filter-with_filter) > tol ||
						(no_filter-with_filter) < 0.0f)
					{
			...
			Debug.WriteLine("");
				...
				if(test_tx_filter.StartsWith("TX Filter Test: Failed ("))
					test_tx_filter = test_tx_filter.Substring(0, test_tx_filter.Length-2)+")";
			...
			path += "\\TX Filter";
			if(!Directory.Exists(path)) Directory.CreateDirectory(path);
			writer = new StreamWriter(path+"\\tx_filter_"+FWCEEPROM.SerialToString(FWCEEPROM.TRXSerial)+".csv");
			writer.WriteLine("Band, Off, On, Diff");
			for(int i=0; i<bands.Length; i++)
			{
				writer.Write(BandToString(bands[i])+",");
		...
		{
			float tol = -55.0f;		// from null to noise floor
			...
			if(test_tx_image.StartsWith("TX Image Test: Failed ("))
				test_tx_image = test_tx_image.Substring(0, test_tx_image.Length-2)+")";
			...
			path += "\\TX Image";
			if(!Directory.Exists(path))	Directory.CreateDirectory(path);
			writer = new StreamWriter(path+"\\tx_image_"+FWCEEPROM.SerialToString(FWCEEPROM.TRXSerial)+".csv");
			...
			FWCEEPROM.WriteTXImage(console.tx_image_gain_table, console.tx_image_phase_table);
			console.FLEX5000SyncDateTime();
		...
		{			
			...
			int reg_15 = 0; // set input side of mixer
			FWC.ReadCodecReg(0x15, out reg_15);
			FWC.WriteCodecReg(0x15, 0x30);
			...
			Audio.RXOutputSignal = Audio.SignalSource.SILENCE;
			Thread.Sleep(500);
			...
			Audio.RXOutputSignal = Audio.SignalSource.SINE;
			Thread.Sleep(500);
			...
			Audio.RXOutputSignal = Audio.SignalSource.SAWTOOTH;
			Thread.Sleep(500);
			...
			// end
			Audio.RXOutputSignal = Audio.SignalSource.RADIO;
		...
		private void CheckIOFWInOut()
		{
			...
			int reg_13 = 0; // set input side of mixer
			FWC.ReadCodecReg(0x13, out reg_13);
			FWC.WriteCodecReg(0x13, 0x30);
		...
		private void CheckIOHeadphone()
		{
			...
			int reg_16 = 0; // set input side of mixer
			FWC.ReadCodecReg(0x16, out reg_16);
			FWC.WriteCodecReg(0x16, 0x30);
			...
			Audio.RXOutputSignal = Audio.SignalSource.SINE_LEFT_ONLY; // check left
			Thread.Sleep(1000);
			...
			Audio.RXOutputSignal = Audio.SignalSource.SINE_RIGHT_ONLY; // check right
			Thread.Sleep(1000);
		...
		private void CheckIODot()
		{
			...
			bool dummy1, dummy2, dummy3, dot;
			FWC.ReadPTT(out dot, out dummy1, out dummy2, out dummy3);
			if(dot) 
			{
				retval = false;
				goto end;
		...
		private void CheckIODash()
		{
			...
			bool dummy1, dummy2, dummy3, dash;
			FWC.ReadPTT(out dummy1, out dash, out dummy2, out dummy3);
			if(dash) 
			{
				retval = false;
				goto end;
		...
		private void CheckIORCAPTT()
		{
			...
			bool dummy1, dummy2, dummy3, ptt;
			FWC.ReadPTT(out dummy1, out dummy2, out ptt, out dummy3);
			if(ptt) 
			{
				retval = false;
				goto end;
		...
		private void CheckIOMicPTT()
		{
			...
			bool dummy1, dummy2, dummy3, ptt;
			FWC.ReadPTT(out dummy1, out dummy2, out dummy3, out ptt);
			if(ptt) 
			{
				retval = false;
				goto end;
		...
		private void PostFenceTest()
		{
			...
			DSPMode dsp_mode = console.CurrentDSPMode;
			Filter filter = console.CurrentFilter;
			int rx_filt_high = console.FilterHighValue;
			int rx_filt_low = console.FilterLowValue;	
			...
			if(filter == Filter.VAR1 || filter == Filter.VAR2)
			{
				console.FilterHighValue = rx_filt_high;
				console.FilterLowValue = rx_filt_low;			
			}
			console.RITOn = rit_on;
