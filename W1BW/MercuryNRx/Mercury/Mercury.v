...
/* 	This program interfaces the LT2208 to a PC over USB.
	...
	Change log:
	 ...
	 20 Aug 2009 - Added C&C Address = 1, changed C&C min speed to 5000
                 - release as V2.8			
     27 Aug 2009 - 1. addition of Merc_ID to uniquely identify multiple Mercury boards
                   2. addition of multiple MDOUT signals to Ozy for multi receiver support
                   3. Channels_1_8 signal addition for determining whether this Mercury
                      card will drive 8 channels (single Mercury card) or just 1 channel
                      (when using multiple cards)
                   4. 3 recievers/filters - change by setting localparam NR
                      Only 3 using current CIC design can physically be packed into Mercury
     24 Sep 2009   1. Tristate A12, A6, C23 if Merc_ID != 3'b000
...
module Mercury(OSC_10MHZ, ext_10MHZ,AUX_CLK, C122_clk,INA,CC,ATTRLY,A6,A12,C21,C23,C24,MDOUT,CDIN,TLV320_BCLK,TLV320_LRCIN,
               ...
               Merc_ID, Merc_ID_drv, Channels_8_1); 
...
output wire  [7:0] MDOUT;          // I/Q out to Atlas bus on {C25, C27, C29, C31, A7, A8, A9, A10}  
...
input  wire        Channels_8_1;   // GPIO 3
...
assign Mercury_Addr = Merc_ID + 1'b1;  // 1 - 8
...
assign ext_10MHZ = (ref_ext & (Merc_ID == 3'b000)) ? OSC_10MHZ : 1'bz; // C16 is bidirectional so set high Z if input. 
...
generate
  genvar c;
  for (c = 0; c < NR; c = c + 1) // !!!!WARNING: only 4 channels right now!!!
...
NWire_xmit #(.SEND_FREQ(48000), .OSL(16), .OSW(1), .ICLK_FREQ(122880000), .XCLK_FREQ(122880000))
        SPD (.irst(C122_rst), .iclk(C122_clk), .xrst(C122_rst), .xclk(C122_clk),
             .xdata(spd_data), .xreq(spd_req), .xrdy(spd_rdy), .xack(spd_ack), .dout(spd_out));

assign A12 = (Merc_ID == 3'b000) ? spd_out : 1'bz;
...
FIFO #(4096) SPF (.rst(C122_rst),    .clk (C122_clk),  .full(spf_full),   .empty(spf_empty),
                  .wrreq (spf_wreq), .data (temp_ADC), .rdreq (spf_rreq), .q(spd_data) );
...
/*
	...
	<[60:59]DFS1,DFS0><[58]PTT><[57:54]address><[53:22]frequency><[21:18]clock_select>
	<[17:11]OC><[10]Mode><[9]PGA><[8]DITHER><[7]RAND><[6:5]ATTEN><[4:3]TX_relay>
	<[2]Rout><[1:0]RX_relay> 
...
assign fn = C122_rcv_data[57:54]; // which Mercury frequemcy 1 - 8 

generate
  genvar j;
  for (j = 0; j < NR; j = j + 1) // capture NR frequencies from Ozy
  begin: Fsave
    always @ (posedge C122_clk)
    begin
      if (C122_rst)
...
begin
  ...
  begin
    ...
    C122_DFS1     <= C122_rcv_data[60];
    C122_DFS0     <= C122_rcv_data[59];    // I/Q sampling rate selection
    C122_PTT_out  <= C122_rcv_data[58];    // PTT from PC via USB 
    if (C122_rcv_data[57:54] == Mercury_Addr) // check that the C&C data is for this board
    begin
      clock_select      <= C122_rcv_data[21:18];     
      //OC               <= C122_rcv_data[17:11]; // Penelope Open Collectors, not used by Mercury
      ATTRLY            <= ~C122_rcv_data[9];    // 1 = Attenuator on, 0 = Preamp on 
      DITHER            <= C122_rcv_data[8];     // 1 = dither on
      RAND              <= C122_rcv_data[7];     // 1 = randomizer on 
      C122_ATTEN        <= C122_rcv_data[6:5];   // Attenuator setting on Alex
      C122_TX_relay     <= C122_rcv_data[4:3];   // Tx relay selection on Alex
      C122_Rout         <= C122_rcv_data[2];     // Rx_1_out on Alex
      C122_RX_relay     <= C122_rcv_data[1:0];   // Rx relay selection on Alex
    end
  end
  else
    C122_new_data <= 1'b0;
...
NWire_rcv  #(.OSL(61), .OSW(1), .ICLK_FREQ(122880000), .XCLK_FREQ(122880000), .SLOWEST_FREQ(5000)) 
...
reg [31:0] C122_frequency_plus_IF; // add PowerSDR IF frequecy (9kHz) to current frequency

always @(posedge C122_clk) // register this addition to avoid timing analysis speed problem
  C122_frequency_plus_IF <= C122_frequency_HZ[0] + 32'd9000; // add 9kHz IF offset 

LPF_select Alex_LPF_select(.clock(C122_clk), .frequency(C122_frequency_plus_IF), .LPF(C122_LPF));
HPF_select Alex_HPF_select(.clock(C122_clk), .frequency(C122_frequency_plus_IF), .HPF(C122_select_HPF));
...
assign C122_6m_preamp = (C122_frequency_plus_IF > 50000000) ? 1'b1 : 1'b0;
