...
//			t.Start();
		...
		private void ControlList(Control c, ref ArrayList a)
		{
			...
			if(c.GetType() == typeof(CheckBoxTS) || c.GetType() == typeof(CheckBox) ||
				c.GetType() == typeof(ComboBoxTS) || c.GetType() == typeof(ComboBoxTS) ||
				...
				c.GetType() == typeof(LabelTS) || c.GetType() == typeof(LabelTS) ||
		...
		public void SaveSettings()
		{
			...
			// pages on this form of the following types: CheckBox, ComboBoxTS,
			...
			foreach(Control c in temp)				// For each control
			{
				...
				else if(c.GetType() == typeof(ColorButton))
				{
					...
					c.GetType() == typeof(LabelTS))
					Debug.WriteLine(c.Name+" needs to be converted to a Thread Safe control.");
#endif
			}

			DB.SaveVars("UCB", ref a);		// save the values to the DB
		}

		public void GetSettings()
		{
			// Automatically restores all controls from the database in the
			// tab pages on this form of the following types: CheckBox, ComboBoxTS,
			// NumericUpDown, RadioButton, TextBox, and TrackBar (slider)
			...
			ArrayList a = DB.GetVars("UCB");						// Get the saved list of controls
			a.Sort();
			...
			foreach(string s in a)				// string is in the format "name,value"
			{
				...
				else if(s.StartsWith("lbl"))
				{
					for(int i=0; i<label_list.Count; i++)
					{
						LabelTS c = (LabelTS)label_list[i];
						if(c.Name.Equals(name))		// name found
						{
							c.Text = val;
							i = label_list.Count+1;
						}
						if(i == checkbox_list.Count)
							MessageBox.Show("Control not found: "+name);
					}
				}
				else if(s.StartsWith("combo"))	// control is a ComboBoxTS
				{
		...
		{
			string s = InputBox.Show("New LabelTS", "Enter a new LabelTS and press OK.", "<new LabelTS>");
