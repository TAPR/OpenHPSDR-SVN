	...
	public class ProductionTest : System.Windows.Forms.Form
	{
		...
		private void ControlList(Control c, ref ArrayList a)
		{
			...
			if(c.GetType() == typeof(CheckBoxTS) || c.GetType() == typeof(CheckBoxTS) ||
				...
				c.GetType() == typeof(ColorButton) || c.GetType() == typeof(ButtonTS))
				a.Add(c);
		}
		...
		public void SaveSettings()
		{
			...
			DB.SaveVars("ProdTest", ref a);		// save the values to the DB
		}
		...
		public void RestoreSettings()
		{
			...
			ArrayList a = DB.GetVars("ProdTest");				// Get the saved list of controls
			a.Sort();
...
//			NoiseTest();
		...
		private void SignalTest()
		{
			...
			console.CurrentFilter = Filter.F2600;
			...
			for(int i=0; i<freq.Length; i++)
			{
				if(!((CheckBox)chk[i]).Checked)
				{
					...
					for(int j=0; j<100; j++)
					{
						...
						num += (console.multimeter_cal_offset + console.filter_size_cal_offset + console.PreampOffset);
						level_sum += num;
		...
		private void NoiseTest()
		{
			...
			console.CurrentFilter = Filter.F500;
			console.CurrentPreampMode = PreampMode.HIGH;
		...
		private void PreampTest()
		{
			...
			for(int i=0; i<50; i++)
			{
				off_level_sum += DttSP.CalculateMeter(DttSP.MeterType.SIGNAL_STRENGTH);
				Thread.Sleep(50);
				if(!progress.Visible) goto end;
			...
			// Preamp Low
			console.CurrentPreampMode = PreampMode.LOW;
			Thread.Sleep(DELAY);
			float low_level_sum = 0f;
			for(int i=0; i<50; i++)
			{
				low_level_sum += DttSP.CalculateMeter(DttSP.MeterType.SIGNAL_STRENGTH);
				Thread.Sleep(50);
				if(!progress.Visible) goto end;
			...
			// Preamp Med
			console.CurrentPreampMode = PreampMode.MED;
			Thread.Sleep(DELAY);
			float med_level_sum = 0f;
			for(int i=0; i<50; i++)
			{
				med_level_sum += DttSP.CalculateMeter(DttSP.MeterType.SIGNAL_STRENGTH);
				Thread.Sleep(50);
				if(!progress.Visible) goto end;
			...
			for(int i=0; i<50; i++)
			{
				high_level_sum += DttSP.CalculateMeter(DttSP.MeterType.SIGNAL_STRENGTH);
				Thread.Sleep(50);
				if(!progress.Visible) goto end;
		...
		private void ImpulseTest()
		{
			...
			console.FilterLowValue = 7500;
			console.FilterHighValue = 8500;
			console.CurrentPreampMode = PreampMode.OFF;
			...
			float level = DttSP.CalculateMeter(DttSP.MeterType.AVG_SIGNAL_STRENGTH);
			...
			float impulse = DttSP.CalculateMeter(DttSP.MeterType.AVG_SIGNAL_STRENGTH);
		...
		private void MicTest()
		{
			...
			float start = (float)Math.Max(-20.0, -DttSP.CalculateMeter(DttSP.MeterType.MIC));
			start = (float)Math.Max(-20.0, -DttSP.CalculateMeter(DttSP.MeterType.MIC));

			// begin outputting tone
			Audio.CurrentOutputSignal = Audio.SignalSource.SINE;

			Thread.Sleep(1000);
			float end = (float)Math.Max(-20.0, -DttSP.CalculateMeter(DttSP.MeterType.MIC));
			end = (float)Math.Max(-20.0, -DttSP.CalculateMeter(DttSP.MeterType.MIC));
		...
		private void BalanceTest()
		{
			...
			for(int i=0; i<100; i++)
			{
				lsum += DttSP.CalculateMeter(DttSP.MeterType.ADC_IMAG);
				rsum += DttSP.CalculateMeter(DttSP.MeterType.ADC_REAL);
				Thread.Sleep(50);
				if(!progress.Visible) goto end;
				progress.SetPercent((i+1)/100.0f);
			}
