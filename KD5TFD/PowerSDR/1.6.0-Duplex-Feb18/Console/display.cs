...
namespace PowerSDR
{
	...
	class Display
	{
		...
		private static void DrawPhaseGrid(ref Bitmap b, int W, int H)
		{
		...
		private static void DrawScopeGrid(ref Bitmap b, int W, int H)
		{
			Bitmap bmp = new Bitmap(W, H);				// create bitmap
			Graphics g = Graphics.FromImage(bmp);		// create graphics object for drawing
		...
		private static void DrawSpectrumGrid(ref Bitmap b, int W, int H)
		{
		...
		private static void DrawPanadapterGrid(ref Bitmap b, int W, int H)
		{
			Bitmap bmp = new Bitmap(W, H);				// create bitmap
			Graphics g = Graphics.FromImage(bmp);		// create graphics object for drawing
			...
			int low = -10000*console.SampleRate1/48000;					// initialize variables
			int high = 10000*console.SampleRate1/48000;
			int mid_w = W/2;
			int[] step_list = {10, 20, 25, 50};
			int step_power = 1;
			int step_index = 0;
			...
			if(console.MOX)								// initialize right and left drawing limits
			{
			...
			if(low < 0 && high > 0)
			{
				...
				for(int i=1; i<numSteps; i++)
				{
					...
					if(label.Length == 3) xOffset = 7;
					//int offset = (int)(label.Length*4.1);
					if(console.CurrentDSPMode == DSPMode.USB ||
		...
		private static void DrawWaterfallGrid(ref Bitmap b, int W, int H)
		{
			Bitmap bmp = new Bitmap(W, H);				// create bitmap
			Graphics g = Graphics.FromImage(bmp);		// create graphics object for drawing
			...
			if(!console.MOX)					// get left and right drawing limits
			{
		...
		unsafe private static bool DrawScope(ref Graphics g, int W, int H)
		{
			if(data_ready)
			{
				// get new data
				fixed(void *rptr = &new_display_data[0])
					fixed(void *wptr = &current_display_data[0])
						MSCVRT.memcpy(wptr, rptr, BUFFER_SIZE*sizeof(float));
						
				data_ready = false;
			}

			double num_samples = console.ScopeTime*48;
			...
			{
				g.DrawLine(new Pen(grid_text_color), console.DisplayCursorX, 0, console.DisplayCursorX, H);
				g.DrawLine(new Pen(grid_text_color), 0, console.DisplayCursorY, W, console.DisplayCursorY);
			}

			return true;
		}

		unsafe private static bool DrawPhase(ref Graphics g, int W, int H)
		{
		...
		unsafe private static void DrawPhase2(ref Graphics g, int W, int H)
		{
			int num_points = phase_num_pts;
		...
		unsafe static private bool DrawSpectrum(ref Graphics g, int W, int H)
		{
		...
		unsafe static private bool DrawPanadapter(ref Graphics g, int W, int H)
		{
			Point[] points = new Point[W];			// array of points to display
			float slope = 0.0F;						// samples to process per pixel
			int num_samples = 0;					// number of samples to process
			int start_sample_index = 0;				// index to begin looking at samples
			int Low = -10000*console.SampleRate1/48000;
			int High = 10000*console.SampleRate1/48000;
			int yRange = spectrum_grid_max - spectrum_grid_min;
		...
		unsafe static private bool DrawWaterfall(ref Graphics g, int W, int H)
		{
			...
			for(int i=0; i<W; i++)
			{
				...
				else 
				{
					...
					for(int j=lindex;j<rindex;j++)
					{
						if (current_display_data[j] > max) 
							max=current_display_data[j];
					}
				}
		...
		unsafe static private bool DrawHistogram(ref Graphics g, int W, int H)
		{
			Point[] points = new Point[W];			// array of points to display
			float slope = 0.0F;						// samples to process per pixel
			int num_samples = 0;					// number of samples to process
