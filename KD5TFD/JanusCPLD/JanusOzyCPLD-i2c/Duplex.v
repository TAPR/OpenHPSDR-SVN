// V1.0 1st May 2006
// 
// Full duplex Xylo interface for Cirrus Logic CS5381 A/D converter and TI TLV320AIC23B A/D & D/A converters
//
// TLV320 is set in slave  mode  and obtains all its clocks from the CS5381 which is hard wired in master mode.


// Sends 48k or 96k 24 bit A/D data and 48k/16 bit mic/line data  via the FX2 FIFO 4
// and receives 48k/16 bit L/R and I/Q data. Control and Command (C&C) data is also send to, and received
// from the PC. The full specification of the C&C comands are contained in the document (****).
...
//
// where Cn is a control byte as follows:
//
// C0 bit 0 = 1 when PTT or dot key  activated or set by MOX in PowerSDR
// C0 bit 1 = 1 when dash key activated. 
// etc....see protocol design document for full C&C format. 

// The CS5381 is hard wired to send Left Justified data from its SDOUT pin
// A counter is used to count the chips BCLK and data is sent to the FX2 USB chip
...
// The format for C&C and D/A data from the PC is a similar  control sequence followed by  48k/16 bit data:-
...
//
// C0 bit 0 = 1 when MOX is set in PowerSDR
// etc....see protocol design document for full C&C format. 


//
// A/D data is in 2's complement format. 
// CS5381  is set via M0 and M1 pins to be 192,96 or 48kHz. 
//
...
//(			- 36 10 5C - Clock/2, 96k, Normal mode)


// The CS5381 and TLV320's Master clock (MCLK) is 24.576MHz (AD_clock on pin 66) and 
// is divided by two within the chips. 
...
//
// Single bit PWM D/A converters using 16 bits are used to generate the I and Q signals.
//
// 
// Built with Quartus II v5.1 
//
// Change log: 	First version 1st May 2006. 
//
// 	
////////////////////////////////////////////////////
//   CS5381 and LTV320AIC23B connections to Xylo board
//
...
//   dbgout          - pin 89
//	 dot			 - pin 73 - PTT and CW dot key - active low 
//	 dash			 - pin 75 - CW dash key - active low 
//   ad_reset        - pin 70 - reset pin on CS5381 chip - active low (chip held in reset while low)    
//
...
reg dbgout;
input dot; 					// PTT or CW dot key, active low
input dash;					// CW dash key, active low
output ad_reset;
reg ad_reset;				// resets CS5381 
...
/*
	The CS5381 A/D is in Master mode and  provides 
...
/* 
	...
	If PTT or dot is active then set Rx_control_0[0] = 1
	If dash is active then set RX_control_0[1] = 1
...
// state 1 - loop until LRCLK goes high
	3'h1: if (LRCLK) 			  			// wait for A/D LRCLK to go high
		...
		if (loop_counter == 63) loop_counter <= 0; 
			count <= 0;
			q <= q;
			if (Rx_control_0[0]) PTT <= 1'b1;	// check for PTT or dot active 
			else PTT <= 1'b0; 
			state_A_D <= 3'h2;
		end
	   else
...
// state 2 - left shift A/D data, check count and take action if necessary
	3'h2: if (!BCLK) state_A_D <= 3'h2; 			// wait for A/D BCLK clock to go high
		 ...
		 begin
		Tx_q[7:0] <= {Tx_q[6:0], TX_DOUT};		// shift current CS5381 data left and add new data
		q[7:0] <= {q[6:0],DOUT};				// ditto for TLV320 
