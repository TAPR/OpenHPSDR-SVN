// V4.3 26th May 2006
//
// (C) Phil Harman VK6APH and  Bill Tracey  KD5TFD 2006.

// ***********  NOTE:  Wolfson and TLV320 are in I2S mode - use correct I2C to set up ***********
// ***********  WOLFSON in Master mode ****************

// Full duplex Xylo interface for Wolfson  and TI TLV320AIC23B A/D converters

// TLV320 is set in slave  mode at 48kHz and obtains all its clocks from the 24.576MHz crystal
// 
//
...
//
// Wolfson speed is set with the following command FROM the PC:
//
// When C0 = 0b0000_000x   
// then 
...
// Wolfson is set be 192/96/48kHz, 24 bit and Left Justify mode. 
//
// TLV320 is set via I2C to be 48kHz, 16 bit and Left Justify mode VIA I2C vis: 
//
...
// Wolfson and TLV320 MCLK clock is 24.576MHz (AD_clock on pin 66) but is divided by two within the chips. 
...
//				TLV put in Slave mode and its clocks obtained from the  CS5381 14th Feb 2006
...
//				Software modified to allow 192k operation - 4th May 2006.
//				Speed control from PC and TLV320 always clocked at 48k added - 15th May 2006.
//				Clocks for TLV320 derived from 24.576MHz crystal at 48k - 21st May 2006
//				Wolfson A/D interface changed to I2S - 21st May 2006.
//				Added TX_wait and RX_wait counters  - 21st May 2006.
//				Start of I2S interface for TLV320 - 22nd May 2006.
//				I2S interface for TLV320 implemented - 23rd May 2006. 
//				
// 	
////////////////////////////////////////////////////
//   Wolfson and LTV320AIC23B connections to Xylo board
//
...
reg [7:0] register;		  	// Wolfson A/D uses this to send its data to Tx FIFO
...
always @ (posedge AD_CLK) begin
        ...
        if ( ad_reset == 0 && ad_count[23] ) begin
             ad_reset <= 1;  // turn on the CS5381
        end
end
...
/*
	...
	The speed the Wolfson runs at, either 192k,96k or 48k is set by 
	...
	Since the Wolfson can run at any of these speeds, and the TLV and PCM  clocks
	are derived from it, then we need to divide these clocks for the
	TLV and PWM D/A  so that they can always run at 48k irrespective of the
	speed of the Wolfson.
	
	The following code divides the BCLK and LRCLK clocks as required. Note the phase relationship
	required betweeen LRCLK and BCLK. LRCLK must change state on the negative edge of BCLK.  
...
// divide MCLK (24.576MHz) to give 48k clocks for the TLV320
...
clocks clocks(AD_CLK, clock_out);

assign TX_BCLK = clock_out[2] ;  	// 3.072MHz
assign TX_LRCLK = clock_out[8];		// 48kHz
assign TLV_BCLK = clock_out[2];		// 3.072MHz
assign TLV_LRCLK = clock_out[8]; 	// 48kHz
assign RX_INLRCLK = TX_LRCLK;

/*

//////////////////////////////////////////////////////////////
//
//	Decode A/D speed (not used with Wolfson in Master mode)
//
//////////////////////////////////////////////////////////////

// The following code decodes the A/D speed from the PC and 
// selects the correct  for the AK5394A.

always @(Rx_control_1[0] or Rx_control_1[1])
begin
if (Rx_control_0[7:1] == 8'd0) // speed data is not valid otherwise
begin
case ({Rx_control_1[1],Rx_control_1[0]})
...
default: begin DFS0 <= 0; DFS1 <= 0; <= 0; end
endcase
end
else begin 
		DFS0 <= DFS0;	// else leave where it is
		DFS1 <= DFS1;
	 end
end
*/			
...
/* 
	The following  code sends the sync bytes, contol bytes and A/D samples. 
	
	The code is structured round counting the number of BCLKs that occur following 
	the positive edge of the LRCLK. At the positive edge a state counter (AD_state) 
	increments at the positive edge of each BCLK.  At each state we determine 
	what data needs to to be latched into a 512 byte Tx FIFO. 
...
always @ (posedge BCLK)
begin 
		q[7:0] <= {q[6:0],DOUT};				// shift current Wolfson data left and add next bit
...
6'd3,6'd4:	begin
		if(loop_counter == 0) begin  			// send remainder of sync 
			register <= 8'h0;
			strobe <= 1'b1;
			end
		AD_state <= AD_state + 1'b1;		
...
6'd6:	begin
		if(loop_counter == 0)begin
			register <= RX_wait;				// C1 - RX wait loop counter
			strobe <= 1'b1;
			end
		AD_state <= AD_state + 1'b1;
		end
6'd7:	begin
		if(loop_counter == 0)begin
			register <= TX_wait;				// C2 - Tx wait loop counter
			strobe <= 1'b1;
			end
		AD_state <= AD_state + 1'b1;
		end 
6'd8:	begin
		if(loop_counter == 0)begin
			register[7:0] <= rx_avail[11:4]; // C3 - number of bytes in Rx FIFO
			strobe <= 1'b1;
			end
		AD_state <= AD_state + 1'b1;
		end 
6'd9:	begin
		if(loop_counter == 0)begin
			register <= Rx_control_4;		// C4 - sequence number
			strobe <= 1'b1;
			end
		AD_state <= AD_state + 1'b1;
		end 
6'd11:	begin
		register <= q;		// MSB of Wolfson 
		strobe <= 1'b1;
		AD_state <= AD_state + 1'b1;
		end
6'd19:	begin
		register <= q;		// Wolfson data 
		strobe <= 1'b1;
		AD_state <= AD_state + 1'b1;
		end
6'd27,6'd38,6'd46,6'd54: begin // Wolfson data at these counts 
		register <= q;
		strobe <= 1'b1;
		AD_state <= AD_state + 1'b1;
		end
...
6'd55:	begin
		register <= Tx_MSB;	// send MSB microphone or line in data to Tx FIFO
		strobe <= 1'b1;
		AD_state <= AD_state + 1'b1;
		end
6'd56:	begin
		register <= Tx_LSB; // send LSB microphone or line in data to Tx FIFO
		strobe <= 1'b1;
		AD_state <= AD_state + 1'b1;
		end
6'd57:	begin
		loop_counter <= loop_counter + 1'b1; // at end off loop so increment loop counter
...
//////////////////////////////////////////////////////////////
//
//	Read TLV320 A/D converter for microphone and line in
//
//////////////////////////////////////////////////////////////

/* 	Since the TLV320 always runs at 48k we need a separate 
	routine to read its data
*/

reg [4:0]TX_state;

always @ (posedge TLV_BCLK)
begin 
 		Tx_q[7:0] <= {Tx_q[6:0], TX_DOUT};			// shift current TLV320 data left and add next bit
case (TX_state)
5'd0:	begin
		if(TLV_LRCLK) TX_state <= 5'd0;				// loop until TX_LRCLK is low
		else TX_state <= 5'd1;
		end
5'd1:	begin
		if(TLV_LRCLK) TX_state <= TX_state + 1'b1; 	// loop until TX_LRCLK is high 
		else TX_state <= 5'd1;
		end
5'd10:	begin
		Tx_MSB <= Tx_q;								// MSB of TLV320 (microphone or line in)
		TX_state <= TX_state + 1'b1;
		end
5'd18:	begin
		Tx_LSB <= Tx_q;								// LSB of TLV320 (microphone or line in)
...
wire [7:0] Tx_register; 		// holds data from A/D to send to FX2 
wire Tx_write_full;				
wire Tx_write_empty; 
...
/*
	...
	which are control bytes C0-C4. It then concatenates  the next two bytes 
	...
	TLV320  D/A converters. The next two bytes are concatented to form the I data and the next 
	...
	of the A/D converter so is synced on the positive edge of the TX_LRCLK. Further reads
...
always @ (negedge BCLK)			// use negedge to ensure that FIFO data is stable 
begin
...
// state 16 - check that TLV_LRCLK is low 
  16: begin
		fifo_enable <= 1'b0;		    // disable read of dual clock fifo
		if (TLV_LRCLK) begin 
			state_PWM <= 16;		// wait for LRCLK to go low so we are in sync

		end
	    else state_PWM <= 17;
	    end	
// state 17 - wait for leading edge of TLV_LRCLK
  17: begin
		if (TLV_LRCLK) 			  			// wait for LRCLK to go high so we are in sync
		begin
...
///////////////////////////////////////////////////////////////
//
//		Implements I2S digital audio out,
//		16 bits, two channels, 48kHz  for TLV320AIC23B D/A converter 
//
///////////////////////////////////////////////////////////////


I2SAudioOut  I2SAO(.lrclk_i(TLV_LRCLK), .bclk_i(TLV_BCLK), .left_sample_i(Left_PWM), .right_sample_i(Right_PWM),.outbit_o(RX_DIN)); 
...
/*
	Pin 68 is used as Tx data out to the PC at 115kB. Data to be sent 
