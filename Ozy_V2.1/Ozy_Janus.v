...
// Nov   9,2011 1)  Extended the length of the Atlas broadcast from 84 bits to 87 bits  
...
//					 3) Added the Chirp 1PPS GPS input signal from Atlas A13 and included a  
//						time stamp signal in the LSB of the 16 bits of mic data in accordance  
//						with reception of the 1PPS signal.
...
//    MDOUT	        - Atlas A7,A8,A9,A10 - pins 142,141,139,138 - I&Q from Mercury 
...
module Ozy_Janus(
        ...
        A2, A3, A4, A5, A6, A12, A13, C4, C5, C6, C7, C8, C9, C12, C13, C14, C19, C21, C22, C23, C24,
...
localparam Ozy_serialno = 8'd21;	// Serial number of this version, v2.1 derived from v2.0, multiple Mercs added
...
input wire			   A2;          // NWire Merc4 version and ADC OVERLOAD4
input wire			   A3;          // NWire Merc3 version and ADC OVERLOAD3
input wire           A4;          // NWire Merc2 version and ADC OVERLOAD2
...
assign IF_IQ_Data    = (IF_conf[1] ? IF_M_IQ_Data[IF_chan] : IF_J_IQ_Data);
...
*/
    NWire_rcv #(.DATA_BITS(48), .ICLK_FREQ(144000000), .XCLK_FREQ(48000000), .SLOWEST_FREQ(20000))
          M_IQ (.irst(C144_rst), .iclk(C144_clk), .xrst(IF_rst), .xclk(IF_clk),
                .xrcv_rdy(IF_M_IQ_Data_rdy[0]), .xrcv_ack(IF_tx_IQ_mic_ack),
                .xrcv_data(IF_M_IQ_Data[0]), .din(MDOUT[0]) );
                
    NWire_rcv #(.DATA_BITS(48), .ICLK_FREQ(144000000), .XCLK_FREQ(48000000), .SLOWEST_FREQ(20000))
          M_IQ1 (.irst(C144_rst), .iclk(C144_clk), .xrst(IF_rst), .xclk(IF_clk),
                .xrcv_rdy(IF_M_IQ_Data_rdy[1]), .xrcv_ack(IF_tx_IQ_mic_ack),
                .xrcv_data(IF_M_IQ_Data[1]), .din(MDOUT[1]) );
     NWire_rcv #(.DATA_BITS(48), .ICLK_FREQ(144000000), .XCLK_FREQ(48000000), .SLOWEST_FREQ(20000))
          M_IQ2 (.irst(C144_rst), .iclk(C144_clk), .xrst(IF_rst), .xclk(IF_clk),
                .xrcv_rdy(IF_M_IQ_Data_rdy[2]), .xrcv_ack(IF_tx_IQ_mic_ack),
                .xrcv_data(IF_M_IQ_Data[2]), .din(MDOUT[2]) );
     NWire_rcv #(.DATA_BITS(48), .ICLK_FREQ(144000000), .XCLK_FREQ(48000000), .SLOWEST_FREQ(20000))
          M_IQ3 (.irst(C144_rst), .iclk(C144_clk), .xrst(IF_rst), .xclk(IF_clk),
                .xrcv_rdy(IF_M_IQ_Data_rdy[3]), .xrcv_ack(IF_tx_IQ_mic_ack),
                .xrcv_data(IF_M_IQ_Data[3]), .din(MDOUT[3]) );
...
assign IF_last_chan = IF_conf[1] ? IF_merc : 3'b0; // ?? if Mercury : only 1 if Janus
...
begin 
  ...
  begin // set up default values - 0 for now
    ...
    IF_merc            <= 3'b000;  // max number of channels to send to PC, 3'b000 => one channel
  ...
  begin 								// Need to ensure that C&C data is stable 
    ...
    begin
      ...
      IF_merc             <= IF_Rx_ctrl_4[5:3]; // decode number of IQ streams to send to the PC
...
/*
	...
	Total of 87 bits. Frequency is in Hz and OC is the open collector data on Penelope.
...
*/
wire [86:0] IF_xmit_data;
reg   [3:0] CC_address;     // C&C address  0 - 7
reg   [2:0] multiRx_mode;   // 000 = single receiver, 001 = two receivers, 010 = three receivers, 011 = four receivers 
...
begin
  ...
  multiRx_mode <= #IF_TPD IF_last_chan[2:0]; //number of receivers requested by the PC

  end

assign IF_xmit_data = {IF_DFS1,IF_DFS0,PTT_out, CC_address,IF_frequency[CC_address],IF_clock_s,IF_OC,IF_mode,IF_PGA,
                       IF_DITHER, IF_RAND, IF_ATTEN, IF_TX_relay, IF_Rout, IF_RX_relay, IF_Alex, IF_Drive_Level, multiRx_mode};

NWire_xmit  #(.DATA_BITS(87), .ICLK_FREQ(48000000), .XCLK_FREQ(48000000), .SEND_FREQ(10000)) 
...
// format. Sends ADC power levels: Penny, FWD and REV as 12 bits

// Format is: <Penny(Lane) Serial #>< Penny power>< FWD >< REV>

wire [43:0] IF_pd;
wire        IF_pd_rdy;
reg [11:0] FWD;			// Forward Power from Penny(Lane)
reg [11:0] REV; 			// REverse Power from Penny(Lane)
...
NWire_rcv  #(.DATA_BITS(9), .ICLK_FREQ(48000000), .XCLK_FREQ(48000000), .SLOWEST_FREQ(500)) 
      m_ver2 (.irst(IF_rst), .iclk(IF_clk), .xrst(IF_rst), .xclk(IF_clk),
             .xrcv_data(IF_md2), .xrcv_rdy(IF_md_rdy2), .xrcv_ack(IF_md_rdy2), .din(A4));
NWire_rcv  #(.DATA_BITS(9), .ICLK_FREQ(48000000), .XCLK_FREQ(48000000), .SLOWEST_FREQ(500)) 
      m_ver3 (.irst(IF_rst), .iclk(IF_clk), .xrst(IF_rst), .xclk(IF_clk),
             .xrcv_data(IF_md3), .xrcv_rdy(IF_md_rdy3), .xrcv_ack(IF_md_rdy3), .din(A3));
NWire_rcv  #(.DATA_BITS(9), .ICLK_FREQ(48000000), .XCLK_FREQ(48000000), .SLOWEST_FREQ(500)) 
      m_ver4 (.irst(IF_rst), .iclk(IF_clk), .xrst(IF_rst), .xclk(IF_clk),
             .xrcv_data(IF_md4), .xrcv_rdy(IF_md_rdy4), .xrcv_ack(IF_md_rdy4), .din(A2));
