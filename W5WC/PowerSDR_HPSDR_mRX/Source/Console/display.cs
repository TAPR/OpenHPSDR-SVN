...
namespace PowerSDR
{
    ...
    class Display
    {
        ...
        private static int rx1_spectrum_filter_low;
        public static int RX1SpectrumFilterLow
        {
            get { return rx1_spectrum_filter_low; }
            set { rx1_spectrum_filter_low = value; }
        }

        private static int rx1_spectrum_filter_high;
        public static int RX1SpectrumFilterHigh
        {
            get { return rx1_spectrum_filter_high; }
            set { rx1_spectrum_filter_high = value; }
        }

        private static int rx2_spectrum_filter_low;
        public static int RX2SpectrumFilterLow
        {
            get { return rx2_spectrum_filter_low; }
            set { rx2_spectrum_filter_low = value; }
        }

        private static int rx2_spectrum_filter_high;
        public static int RX2SpectrumFilterHigh
        {
            get { return rx2_spectrum_filter_high; }
            set { rx2_spectrum_filter_high = value; }
        }

        private static int tx_spectrum_filter_low;
        public static int TXSpectrumFilterLow
        {
            get { return tx_spectrum_filter_low; }
            set { tx_spectrum_filter_low = value; }
        }

        private static int tx_spectrum_filter_high;
        public static int TXSpectrumFilterHigh
        {
            get { return tx_spectrum_filter_high; }
            set { tx_spectrum_filter_high = value; }
        }
...
*/
        ...
        unsafe private static void DrawPanadapterGrid(ref Graphics g, int W, int H, int rx, bool bottom)
        {
            ...
            if (!local_mox)// && (rx1_dsp_mode == DSPMode.CWL || rx1_dsp_mode == DSPMode.CWU))
            {
                ...
                double f_vfob = vfob_hz;
                double f_lowb = f_vfob + filter_low;
                double f_highb = f_vfob + filter_high;

                f_vfob -= vfob_hz;
                f_lowb -= vfob_hz;
                f_highb -= vfob_hz;
                r_x0[1] = (int)Math.Round((f_vfob - Low) / width * W);
                r_x1[1] = (int)Math.Round((f_lowb - Low) / width * W);
                r_x2[1] = (int)Math.Round((f_highb - Low) / width * W);
                ...
                if (bottom)
                {
                    ...
                    //	filter_left_x, H + top, filter_right_x-filter_left_x, H + H - top);
                    g.FillRectangle(display_filter_brush, r_x1[1], H + top, r_x2[1] - r_x1[1], H + H - top);
                }
                else
                {
           ...
           // if (mox && tx_grid_control) DrawTXPanadapterGrid(ref g, W, H, rx, bottom);
            ...
            if (rx == 1 && data_ready)
            {
                if (local_mox && (rx1_dsp_mode == DSPMode.CWL || rx1_dsp_mode == DSPMode.CWU))
                {
                    ...
                    for (int i = 0; i < current_display_data.Length; i++)
                        current_display_data[i] = spectrum_grid_min - rx1_display_cal_offset; //-200.0f;
                }
                else
                {
                    fixed (void* rptr = &new_display_data[0])
            ...
            for (int i = 0; i < W; i++)
            {
                ...
                if (rx == 1)
                {
                    ...
                    {
                        for (int j = lindex; j < rindex; j++)
                            if (current_display_data[j % 4096] > max)
                                max = current_display_data[j % 4096];
                    }
                }
                else if (rx == 2)
                {
                    if (slope <= 1.0 || lindex == rindex)
                    {
                        max = current_display_data_bottom[lindex % 4096] * ((float)lindex - dval + 1) +
                            current_display_data_bottom[(lindex + 1) % 4096] * (dval - (float)lindex);
                    }
                    else
                    {
                        for (int j = lindex; j < rindex; j++)
                            if (current_display_data_bottom[j % 4096] > max)
            ...
            if (console.PowerOn)
            {
                ...
                if ((duration > waterfall_update_period) || (duration < 0))
                {
                    ...
                    for (int i = 0; i < W; i++)
                    {
                        ...
                        {
                            if (!mox) max = current_display_data_bottom[i];
                    ...
                    if (rx == 1)
                    {
                        switch (current_display_mode)   //w3sz added switch for panafall waterfall height
                        {
                            ...
                            default:
                                ...
                                {
                                    bitmapData = waterfall_bmp.LockBits(
                                        new Rectangle(0, 0, waterfall_bmp2.Width, waterfall_bmp.Height / 2),
                                        ImageLockMode.ReadWrite,
