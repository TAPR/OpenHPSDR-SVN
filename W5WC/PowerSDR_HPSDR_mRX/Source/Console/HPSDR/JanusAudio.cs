...
namespace PowerSDR
{
    ...
    public class JanusAudio
    {
        ...
        unsafe public static void SetVFOfreqRX3(double f)
        {
            lastVFORX3freq = f;
            int f_freq = (int)((f * 1e6 * freq_correction_factor) - low_freq_offset);
            SetRX3VFOfreq(f_freq); // low frq
        }

        private static double lastVFORX4freq = 0.0;
        unsafe public static void SetVFOfreqRX4(double f)
        {
            lastVFORX4freq = f;
