//  V1.5 Aug 2, 2009 by Kirk Weedman KD7IRS
//
...
localparam Ozy_serialno = 8'd15;	// Serial number of this version
...
sync_usb  #(3, RX_FIFO_SZ, 64, TX_FIFO_SZ, 256) //RX FIFO = 2048, TX FIFO = 4096
    usb1 (IF_clk, IF_rst, FX2_FD, FLAGA, FLAGC, SLWR, SLRD, SLOE, PKEND, FIFO_ADR,
          IF_Rx_fifo_wdata, IF_Rx_fifo_drdy, IF_Rx_fifo_used, IF_Rx_fifo_full,
          IF_tx_fifo_rreq, IF_tx_fifo_full, IF_tx_fifo_used, IF_tx_fifo_rdata, IF_tx_fifo_empty);
*/
...
begin 
  ...
  begin 								// Need to ensure that C&C data is stable 
    ...
    else if (IF_Rx_ctrl_0[7:1] == 7'b0000_001)   // decode IF_frequency 
      IF_frequency <= {IF_Rx_ctrl_1, IF_Rx_ctrl_2, IF_Rx_ctrl_3, IF_Rx_ctrl_4};
  end
end	
...
assign IF_xmit_data = {IF_DFS1,IF_DFS0,PTT_out,CC_address,IF_frequency,IF_clock_s,IF_OC,IF_mode,IF_PGA,
...
NWire_xmit  #(.DATA_BITS(61), .ICLK_FREQ(48000000), .XCLK_FREQ(48000000), .SEND_FREQ(1000)) 
      CCxmit (.irst(IF_rst), .iclk(IF_clk), .xrst(IF_rst), .xclk(IF_clk),
              .xdata(IF_xmit_data), .xreq(1'b1), .xrdy(), .xack(), .dout(CC));
///////////////////////////////////////////////////////
//
//                      NWire Penelope data
//
