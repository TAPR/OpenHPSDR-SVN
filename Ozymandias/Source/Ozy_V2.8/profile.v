...
// Raised cosine will be applied over 5mS. Profile consists of 500 samples and is read from ROM.
// ROM address is read forwards for key press and backwards for key release.
// Clock is at 96kHz which is an integer multiple of 48kHz so we don't cross clock domains. 
...
module profile (
	input clock,								// 96kHz clock from PLL
...
reg  [17:0] timer = 0;						// Holds start. Each clock = 10.4uS hence 96 clocks = 1mS
reg  char_PTT = 0;							// Max delay = 1023mS so need 96 * 1023 ~= 100,000 = 2^18 counts. 
reg  enable_hang = 0;
...
3: begin
		if (timer == (delay * 96))begin  // extend element by delay time 
			timer <= 0;
			prof_state <= 4;
		end 
		else timer = timer + 18'd1;
