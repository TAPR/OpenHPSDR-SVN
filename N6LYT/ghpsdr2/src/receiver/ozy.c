...
fprintf(stderr,"spectrum_thread: socket %d\n",spectrum_socket);
    ...
    while(1) {
        ...
        bytes_written=sendto(command_socket,output_buffer,sizeof(output_buffer),0,(struct sockaddr *)&audio_addr,audio_length);
        if(bytes_written<0) {
...
fprintf(stderr,"send_command: command='%s'\n",command);
    ...
    rc=sendto(command_socket,command,strlen(command),0,(struct sockaddr *)&server_addr,server_length);
    if(rc<0) {
        fprintf(stderr,"sendto command failed: %d\n",rc);
        exit(1);
    }

    rc=recvfrom(command_socket,response,sizeof(response),0,(struct sockaddr *)&server_addr,&server_length);
    if(rc<0) {
...
void ozyDisconnect() {
    char command[128];
    sprintf(command,"disconnect %d",receiver);
    send_command(command);
}
...
int set_frequency() {
    ...
    result=0;
    sprintf(command,"set frequency %d %lld",receiver,ddsAFrequency);
    send_command(command);
    token=strtok(response," ");
    if(token!=NULL) {
        if(strcmp(token,"OK")==0) {
...
int ozy_init() {
    ...
    command_socket=socket(PF_INET,SOCK_DGRAM,IPPROTO_UDP);
    if(command_socket<0) {
