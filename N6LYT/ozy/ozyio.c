...
struct usb_device *findOzy(struct usb_bus *busp) {
	...
	bp = busp;
	while ( bp != NULL )  {
		if ( bp->root_dev != NULL ) {  /* BobFix */
//BobFix		if ( bp->root_dev == NULL ) { /* original */
...
struct OzyHandle *internalOzyOpen(void)
{
	...
	int rc;
        ...
        //fprintf(stderr,"internalOzyOpen\n");
	...
	if ( !OzyUSBinitialized ) {
                //fprintf(stderr,"internalOzyOpen: usb_init\n");
		usb_init();
		OzyUSBinitialized = 1;
                //sleep(5);
	}
        
        //fprintf(stderr,"internalOzyOpen: usb_find_busses,usb_find_devices,usd_get_devices\n");
	usb_find_busses();
	usb_find_devices();
	busp = usb_get_busses();
        //sleep(5);
        
        //fprintf(stderr, "internalOzyOpenL findOzy\n");
	ozydevp = findOzy(busp);
	if ( ozydevp == NULL ) {
        ...
        //fprintf(stderr,"internalOzyOpen: usb_open\n");
	...
	/* set the configuration */
        //fprintf(stderr,"internalOzyOpen:usb_set_configuration\n");
	...
	/* claim the  interface */
        //fprintf(stderr,"internalOzyOpen: usb_claim_interface\n");
	...
	if ( rc != 0 ) {
        ...
        //fprintf(stderr,"internalOzyOpen: usb_set_altinterface\n");
	...
	/* clear halts */
        //fprintf(stderr,"internalOzyOpen: usb_clear_halt (0x86)\n");
	...
	if ( rc != 0 ) {
        ...
        //fprintf(stderr,"internalOzyOpen: usb_clear_halt (0x02)\n");
...
void *OzyHandleToRealHandle(struct OzyHandle *ozh) {
	return (void *)ozh->h;
}
