/**
* @file bandscope.c
* @brief simple bandscope display
...
static float waterfall_high=-10.0;
static float waterfall_low=-50.0;
...
void init_bandscope() {
    ...
    bs_points=calloc(WIDTH,sizeof(GdkPoint));
    for(i=0;i<WIDTH;i++) {
        bs_points[i].x=i;
        bs_points[i].y=50;
    }
...
/*
    for(i=0;i<BUFFER_SIZE;i++) {
        bs_points[i].x=i;
        bs_points[i].y=(int)(floor((max_display-result[(i+(BUFFER_SIZE/2))%BUFFER_SIZE])*(float)HEIGHT/range));
    }
*/
}
...
void draw_samples() {
    ...
    if(spectrum->window) {
        ...
        gdk_draw_lines(spectrum_pixmap,gc,bs_points,WIDTH);
