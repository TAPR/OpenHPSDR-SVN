  ...
  if (uni[thread].meter.flag)
    {                           // reset metering completely
      ...
      for (i = 0; i < RXMETERPTS; i++)
                for (k = 0; k < MAXRX; k++)
                        uni[thread].meter.rx.val[k][i] = 0.0;
      for (i = 0; i < TXMETERPTS; i++)
                  uni[thread].meter.tx.val[i] = 0.0;
    }
}
...
setup_all (REAL rate,
           int buflen,
           SDRMODE mode, char *wisdom, int specsize, int numrecv, int cpdsize, unsigned int thread)
{
...
setup_rx (int k, unsigned int thread)
{
  ...
  rx[thread][k].dttspagc.gen = newDttSPAgc (agcMED,     // mode kept around for control reasons alone
  ...
  /* demods */
  rx[thread][k].am.gen = newAMD (uni[thread].samplerate,        // REAL samprate
                         0.0,   // REAL f_initial
  ...
  rx[thread][k].fm.gen = newFMD (uni[thread].samplerate,        // REAL samprate
                         0.0,   // REAL f_initial
  ...
  /* noise reduction */
  rx[thread][k].anf.gen = new_lmsr (rx[thread][k].buf.o,        // CXB signal,
                            64, // int delay,
                            0.01f,      // REAL adaptation_rate,
                            0.0001f,   // REAL leakage,
                            45, // int adaptive_filter_size,
                            LMADF_INTERFERENCE);
  rx[thread][k].anf.flag = FALSE;

  rx[thread][k].anr.gen = new_lmsr (rx[thread][k].buf.o,        // CXB signal,
                            64, // int delay,
                            0.01f,      // REAL adaptation_rate,
                            0.0001f,   // REAL leakage,
                            45, // int adaptive_filter_size,
  ...
  rx[thread][k].spot.gen = newSpotToneGen (-12.0,       // gain
                                   700.0,       // freq
                                   5.0, // ms rise
                                   5.0, // ms fall
...
setup_tx (unsigned int thread)
{
  ...
  tx[thread].filt.coef = newFIR_Bandpass_COMPLEX (300.0,
                                          3000.0,
                                          uni[thread].samplerate, uni[thread].buflen + 1);
  ...
  tx[thread].leveler.gen = newDttSPAgc (1,      // mode kept around for control reasons
  ...
  tx[thread].alc.gen = newDttSPAgc (1,  // mode kept around for control reasons alone
                            CXBbase (tx[thread].buf.i), // input buffer
                            CXBsize (tx[thread].buf.i), // output buffer
                            1.2f,       // Target output
                            2,  // Attack time constant in ms
  ...
  switch (tap) {
    ...
    case RXMETER_POST_FILT:
      ...
      uni[thread].meter.rx.val[k][SIGNAL_STRENGTH] =
          ...
          if (uni[thread].meter.rx.mode[k] == SIGNAL_STRENGTH) uni[thread].meter.rx.val[k][AVG_SIGNAL_STRENGTH] = uni[thread].meter.rx.val[k][SIGNAL_STRENGTH];
...
do_rx_post (int k, unsigned int thread)
{
  ...
  if (rx[thread][k].grapheq.flag && rx[thread][k].mode != DRM)
  ...
  // position in stereo field
        ...
        if (!rx[thread][k].bin.flag)
        {
                ...
                {
                        for (i = 0; i < n; i++)
                                CXBdata(rx[thread][k].buf.o, i) = Cscl(rx[thread][k].azim, 1.41421356f*CXBreal(rx[thread][k].buf.o, i));
                }
                else
                {
                        for (i = 0; i < n; i++)
                                CXBdata(rx[thread][k].buf.o, i) = Cscl(rx[thread][k].azim, /*1.2f**/CXBreal(rx[thread][k].buf.o, i));
                }
        }
...
do_rx_SBCW (int k, unsigned int thread)
{
    ...
    if (!rx[thread][k].bin.flag) for (i = 0; i < CXBhave (rx[thread][k].buf.o); i++)
...
do_tx_meter (unsigned int thread, CXB buf, TXMETERTYPE mt)
{
        ...
        switch (mt)
        {
                case TX_MIC:
						...
						micsave = CXBpeak(tx[thread].buf.i);
                        uni[thread].meter.tx.val[TX_MIC] = (REAL) (-10.0 * log10 (micsave + 1e-16));
                        break;
                ...
                case TX_ALC:
                        /*for (i = 0; i < CXBhave (tx[thread].buf.i); i++)
                                ...
                                0.0005 * Csqrmag (CXBdata (tx[thread].buf.i, i)));*/
						alcsave = CXBpeak(tx[thread].buf.i);
                        uni[thread].meter.tx.val[TX_ALC] = (REAL) (-10.0 * log10 (alcsave+ 1e-16));
                ...
                case TX_EQtap:
                        /*for (i = 0; i < CXBhave (tx[thread].buf.i); i++)
                                eqtapsave =
                                        ...
                                        0.0005 * Csqrmag (CXBdata (tx[thread].buf.i, i)));*/
						eqtapsave = CXBpeak(tx[thread].buf.i);
                        uni[thread].meter.tx.val[TX_EQtap] =
                ...
                case TX_LEVELER:
                        /*for (i = 0; i < CXBhave (tx[thread].buf.i); i++)
                                ...
                                0.0005 * Csqrmag (CXBdata (tx[thread].buf.i, i)));*/
						levelersave = CXBpeak(tx[thread].buf.i);
                        uni[thread].meter.tx.val[TX_LEVELER] = (REAL) (-10.0 * log10 (levelersave + 1e-16));
                ...
                case TX_COMP:
                        /*for (i = 0; i < CXBhave (tx[thread].buf.i); i++)
                                ...
                                0.0005 * Csqrmag (CXBdata (tx[thread].buf.i, i)));*/
						compsave = CXBpeak(tx[thread].buf.i);
                        uni[thread].meter.tx.val[TX_COMP] = (REAL) (-10.0 * log10 (compsave+1e-16));
                        break;
                ...
                case TX_CPDR:
                        /*for (i = 0; i < CXBhave (tx[thread].buf.i); i++)
                                ...
                                0.0005 * Csqrmag (CXBdata (tx[thread].buf.i, i)));*/
						cpdrsave = CXBpeak(tx[thread].buf.i);
                        uni[thread].meter.tx.val[TX_CPDR] = (REAL) (-10.0 * log10 (cpdrsave + 1e-16));
                        break;
...
do_tx_pre (unsigned int thread)
{
        ...
        switch(tx[thread].mode)
        {
                ...
                case DIGL:
                case DRM:
                        do_tx_meter (thread, tx[thread].buf.i, TX_EQtap);
                        do_tx_meter (thread, tx[thread].buf.i, TX_LEVELER);
                        do_tx_meter (thread, tx[thread].buf.i, TX_LVL_G);
