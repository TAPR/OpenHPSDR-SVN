  ...
  for (i = 0; i < ssiz; i++)
    {
      ...
      dlay(dptr) = Cmplx(ssig (i),ssig_i(i));
      accum = cxzero;
      sum_sq = cxzero;
	  ...
	  {
          k = wrap (j);
		  sum_sq.re += sqr (dlay(k).re);
		  sum_sq.im += sqr (dlay(k).im);
		  accum.re = afil (j).re * dlay (k).re;
		  accum.im = afil (j).im * dlay (k).im;
	  }

      error.re = ssig (i)- accum.re;
      error.im = ssig_i (i) - accum.im;
      ssig (i) = error.re;
	  ssig_i (i) = error.im;

      scl2.re = (REAL) (rate / (sum_sq.re + 1.19e-7));
      error.re *= scl2.re;
      scl2.im = (REAL) (rate / (sum_sq.im + 1.19e-7));
      error.im *= scl2.im;
      for (j = 0; j < asiz; j++)
	  {
		  k = wrap (j);
		  afil(j) = Cadd(Cscl(afil(j),scl1), Cmplx(error.re * dlay (k).re, error.im * dlay (k).im));
	  }
      dptr = bump (dptr);
    }
}
  ...
  for (i = 0; i < ssiz; i++)
    {
	  ...
	  error = Csub(Cmplx(ssig (i), ssig_i (i)), accum);
      ssig (i) = accum.re;
	  ssig_i (i) = accum.im;
