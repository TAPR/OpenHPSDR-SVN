...
SetRXAGC (unsigned int thread, unsigned subrx, AGCMODE setit)
{
        ...
        rx[thread][subrx].dttspagc.gen->sndx = (int) (uni[thread].samplerate * 0.006f);
        rx[thread][subrx].dttspagc.gen->fastindx = FASTLEAD;
        switch (setit)
    {
                case agcOFF:
                ...
                case agcLONG:
                        ...
                        rx[thread][subrx].dttspagc.gen->fasthangtime = (REAL) 0.1;
                ...
                case agcFAST:
                        ...
                        rx[thread][subrx].dttspagc.gen->hangtime = (REAL) 0.1;
                        rx[thread][subrx].dttspagc.gen->fasthangtime = (REAL) 0.1;
                        rx[thread][subrx].dttspagc.gen->hangtime = (REAL) 0.1;
                        rx[thread][subrx].dttspagc.gen->decay =
...
SetTXLevelerAttack (unsigned int thread, int attack)
{
        ...
        tx[thread].leveler.gen->fasthangtime = (REAL) 0.1;      //wa6ahl: 100 ms
        sem_post(&top[thread].sync.upd.sem);
}
...
DttSP_EXP float
CalculateTXMeter (unsigned int thread, unsigned int subrx, METERTYPE mt)
{
  float returnval = 0;
        //sem_wait (&top[thread].sync.upd.sem);
        ...
        switch(mt) {
                ...
                case ALC:
                        ...
                        break;
                case EQtap:
                        returnval = (float) uni[thread].meter.tx.val[TX_EQtap];
                        break;
                case LEVELER:
                        returnval = (float) uni[thread].meter.tx.val[TX_LEVELER];
                        break;
                case COMP:
