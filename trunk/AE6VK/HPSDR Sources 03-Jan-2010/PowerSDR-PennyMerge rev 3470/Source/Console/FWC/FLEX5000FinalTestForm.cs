...
namespace PowerSDR
{
	public class FLEX5000FinalTestForm : System.Windows.Forms.Form
	{
		...
		{			
			bool b = console.CalibratePABias(progress, (float)udBiasDriverTarget.Value, (float)udBiasFinalTarget.Value, 0.005f, 0);
			if(b)
			{
			...
			if(b)
			{
				FWCEEPROM.WritePABias(console.pa_bias_table);
		...
		private void NullBridge()
		{
			...
			if(pm.Watts*pm_trim[10] < 20.0f)
			{
				FWC.SetMOX(false);
				progress.Hide();
				MessageBox.Show("PA Bridge Null Error: Low Power.  Check capacitors and try again.",
		...
		end3:
			...
			console.CurrentDSPMode = dsp_mode;
			console.FullDuplex = false;
			btnPASWR.BackColor = SystemColors.Control;
			...
			for(int i=0; i<band_freqs.Length; i++)
			{
				...
				if(do_band)
				{
					...
					for(int k=0; k<targets.Length; k++)
					{
						...
						if(Math.Abs(pm_power[i][k]-targets[k]) > 5.0f/* ||
							Math.Abs(1-(bridge_power[i][k]/targets[k])) > tol*/)
						{
							btnPAVerify.BackColor = Color.Red;
		...
		private void PARunCal()
		{
				...
				if(do_band)
				{
					...
					{          
						...
						while(Math.Abs(targets[k]-p) > tol)
						{
							...
							{
								if(try_again_count++ == 12) // poop out
								{
									FWC.SetMOX(false);
									k = 12;
									p = targets[k];									
								...
								if(k != targets.Length-1)
								{
									...
									if(y2 != y1 && x2 != x1)
									{
										...
										if(a > 0.0)
										{
											...
											next_volts = Math.Sqrt((targets[k+1]-b)/a);
											if(double.IsNaN(next_volts))
											{
												// error out -- two times through the cap will do this
												FWC.SetMOX(false);
			...
			/*if(pass) btnPABridge.BackColor = Color.Green;
			else btnPABridge.BackColor = Color.Red;*/
			...
			try
			{
				writer = new StreamWriter("power.csv");
		...
		private void CheckIOXVRX()
		{
			...
			float on = sum / 5;
			on = on + console.MultiMeterCalOffset + console.PreampOffset + console.RXPathOffset;
			...
			float off = sum / 5;
			off = off + console.MultiMeterCalOffset + console.PreampOffset + console.RXPathOffset;
			...
			if(console.CurrentFilter == filter)
				console.UpdateFilters(var_low, var_high);
			else console.CurrentFilter = filter;
		...
		private void CheckIORX1InOut()
		{
			...
			int var_low = console.FilterLowValue;
			int var_high = console.FilterHighValue;
			console.UpdateFilters(-1000, 1000);
