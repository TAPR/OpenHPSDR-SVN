...
namespace PowerSDR
{
	...
	public class RadioDSPRX
	{
		...
		private double nr_leak_dsp = 0.00005;
		private double nr_leak = 0.00005;
		...
		private double anf_leak_dsp = 0.00005;
		private double anf_leak = 0.00005;
	...
	public class RadioDSPTX
	{
		...
		private void SyncAll()
		{
			...
			TXOsc = tx_osc;
			TXAGCFF = tx_agc_ff;
			TXAGCFFCompression = tx_agc_ff_compression;
			DCBlock = dc_block;
		...
		private bool tx_agc_ff_dsp = false;
		private bool tx_agc_ff = false;
		public bool TXAGCFF
		{
			get { return tx_agc_ff; }
			set
			{
				tx_agc_ff = value;
				if(update)
				{
					if(value != tx_agc_ff_dsp || force)
					{
						//DttSP.SetTXAGCFF(thread, value);
						tx_agc_ff_dsp = value;
					}
				}
			}
		...
		private double tx_agc_ff_compression_dsp = 3.0;
		private double tx_agc_ff_compression = 3.0;
		public double TXAGCFFCompression
		{
			get { return tx_agc_ff_compression; }
			set
			{
				tx_agc_ff_compression = value;
				if(update)
				{
					if(value != tx_agc_ff_compression_dsp || force)
					{
						DttSP.SetTXAGCFFCompression(thread, value);
						tx_agc_ff_compression_dsp = value;
					}
				}
			}
		}
