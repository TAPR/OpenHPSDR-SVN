...
{		
	class Display
	{
		...
		private static int rx_filter_low;
		public static int RXFilterLow
		{
			get { return rx_filter_low; }
			set { rx_filter_low = value; }
		}

		private static int rx_filter_high;
		public static int RXFilterHigh
		{
		...
		private static Model current_model = Model.SDR1000;
...
*/
		...
		unsafe public static void RenderGDIPlus(ref PaintEventArgs e)
		{
			...
			if(!split_display)
			{
				switch(current_display_mode) 
				{
					case DisplayMode.SPECTRUM:
						...
						break;
					case DisplayMode.PANADAPTER:
						update = DrawPanadapter(e.Graphics, W, H, false);
						break;
					case DisplayMode.SCOPE:
					...
					case DisplayMode.PHASE2:
						...
						break;
					case DisplayMode.WATERFALL:
						update = DrawWaterfall(e.Graphics, W, H, false);
						break;
					case DisplayMode.HISTOGRAM:
			...
			{
				switch(current_display_mode_top)
				{
					case DisplayMode.SPECTRUM:
						update = DrawSpectrum(e.Graphics, W, H/2, false);
						break;
					case DisplayMode.PANADAPTER:
						update = DrawPanadapter(e.Graphics, W, H/2, false);
						break;
					case DisplayMode.SCOPE:
					...
					case DisplayMode.PHASE2:
						...
						break;
					case DisplayMode.WATERFALL:
						update = DrawWaterfall(e.Graphics, W, H/2, false);
						break;
					case DisplayMode.HISTOGRAM:
				...
				switch(current_display_mode_bottom)
				{
					case DisplayMode.SPECTRUM:
						...
						break;
					case DisplayMode.PANADAPTER:
						update = DrawPanadapter(e.Graphics, W, H/2, true);
						break;
					case DisplayMode.SCOPE:
					...
					case DisplayMode.PHASE2:
						...
						break;
					case DisplayMode.WATERFALL:
						update = DrawWaterfall(e.Graphics, W, H/2, true);
						break;
					case DisplayMode.HISTOGRAM:
						update = DrawHistogram(e.Graphics, W, H/2);
						break;
		...
		private static void DrawPanadapterGrid(ref Graphics g, int W, int H, bool bottom)
		{
			// draw background
			if(bottom) g.FillRectangle(new SolidBrush(display_background_color), 0, H, W, H);
			else g.FillRectangle(new SolidBrush(display_background_color), 0, 0, W, H);
			...
			if(!mox && sub_rx_enabled)
			{
				...
				int filter_left_x = (int)((float)(filter_low-low+vfob_hz-vfoa_hz-rit_hz)/(high-low)*W);
				int filter_right_x = (int)((float)(filter_high-low+vfob_hz-vfoa_hz-rit_hz)/(high-low)*W);
		...
		private static void DrawWaterfallGrid(ref Graphics g, int W, int H, bool bottom)
		{
			// draw background
			if(bottom) g.FillRectangle(new SolidBrush(display_background_color), 0, H, W, H);
			else g.FillRectangle(new SolidBrush(display_background_color), 0, 0, W, H);
			...
			if(!bottom)
			{
					...
					(current_dsp_mode != DSPMode.CWL && current_dsp_mode != DSPMode.CWU))
				{
					...
					{
						filter_left_x = (int)((float)(tx_filter_low-low+xit_hz+(vfob_hz-vfoa_hz))/(high-low)*W);
						filter_right_x = (int)((float)(tx_filter_high-low+xit_hz+(vfob_hz-vfoa_hz))/(high-low)*W);
					}
		...
		unsafe private static bool DrawScope(Graphics g, int W, int H, bool bottom)
		{
			...
			{	
				int pixels = 0;
				if(bottom) pixels = (int)(H/2 * current_display_data_bottom[(int)Math.Floor(i*slope)]);
				else pixels = (int)(H/2 * current_display_data[(int)Math.Floor(i*slope)]);
				int y = H/2 - pixels;
				if(y < max_y)
				{
					max_y = y;
			...
			if(current_click_tune_mode != ClickTuneMode.Off)
			{
				...
				if(bottom) g.DrawLine(p, display_cursor_x, H, display_cursor_x, H+H);
		...
		unsafe static private bool DrawPanadapter(Graphics g, int W, int H, bool bottom)
		{
			DrawPanadapterGrid(ref g, W, H, bottom);
			if(points == null || points.Length < W) 
				points = new Point[W];			// array of points to display
			float slope = 0.0F;						// samples to process per pixel
			int num_samples = 0;					// number of samples to process
		...
		unsafe static private bool DrawWaterfall(Graphics g, int W, int H, bool bottom)
		{
			DrawWaterfallGrid(ref g, W, H, bottom);
			...
			if(bottom) g.DrawImageUnscaled(waterfall_bmp, 0, H+16);
			else g.DrawImageUnscaled(waterfall_bmp, 0, 16);	// draw the image on the background	
