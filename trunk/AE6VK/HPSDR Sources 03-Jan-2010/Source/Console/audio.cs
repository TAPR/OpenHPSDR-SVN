...
namespace PowerSDR
{
	public class Audio
	{
		...
		private static SignalSource rx_input_signal = SignalSource.RADIO;
		public static SignalSource RXInputSignal
		{
			get { return rx_input_signal; }
			set { rx_input_signal = value; }
		}

		private static SignalSource rx_output_signal = SignalSource.RADIO;
		public static SignalSource RXOutputSignal
		{
			get { return rx_output_signal; }
			set { rx_output_signal = value; }
		}
			...
			PA19.PaStreamCallbackTimeInfo* timeInfo, int statusFlags, void *userData)
		{
			...
			switch(current_audio_state1)
			{
				...
				/*case AudioState.SINL_COSR:
					...
					if(two_tone)
					{
						...
						SineWave2Tone(out_l2, frameCount,
							phase_accumulator1, phase_accumulator2,
							sine_freq1, sine_freq2,
							out dump, out dump);
							
						CosineWave2Tone(out_r2, frameCount,
							phase_accumulator1, phase_accumulator2,
							sine_freq1, sine_freq2,
							out phase_accumulator1, out phase_accumulator2);
					}
				...
				case AudioState.SINL_SINR:
					...
					if(two_tone)
					{
						SineWave2Tone(out_l2, frameCount,
							...
							sine_freq1, sine_freq2,
							out phase_accumulator1, out phase_accumulator2);
						CopyBuffer(out_l2, out_r2, frameCount);							
					}
					else
					{
						phase_accumulator1 = SineWave(out_l2, frameCount, phase_accumulator1, sine_freq1);
						CopyBuffer(out_l2, out_r2, frameCount);
					}
					break;
				case AudioState.SINL_NOR:
				...
				case AudioState.COSL_SINR:
					...
					if(two_tone)
					{
						...
						CosineWave2Tone(out_l2, frameCount,
							phase_accumulator1, phase_accumulator2,
							sine_freq1, sine_freq2,
							out dump, out dump);

						SineWave2Tone(out_r2, frameCount,
							phase_accumulator1, phase_accumulator2,
							sine_freq1, sine_freq2,
							out phase_accumulator1, out phase_accumulator2);
					}
					...
					{
						CosineWave(out_l2, frameCount, phase_accumulator1, sine_freq1);
						phase_accumulator1 = SineWave(out_r2, frameCount, phase_accumulator1, sine_freq1);
					}
					break;
				case AudioState.NOL_SINR:
					out_l1 = out_l_ptr1;
			...
			if(full_duplex)
			{
				...
				else
				{
					...
					ScaleBuffer(out_l2, out_l1, frameCount, (float)FWCTXScale);
					ScaleBuffer(out_r2, out_r1, frameCount, (float)FWCTXScale);
		...
		*/
		...
		unsafe private static void InitVAC()
		{
			if(rb_vacOUT_l == null) rb_vacOUT_l = new RingBufferFloat(65536);
			rb_vacOUT_l.Restart(block_size2);
			
			if(rb_vacOUT_r == null) rb_vacOUT_r = new RingBufferFloat(65536);
			rb_vacOUT_r.Restart(block_size2);
		...
		public static bool Start()
		{
			...
			if(vac_enabled)
			{
				...
				vac_rb_reset = true;
				retval = StartAudio_NonJanus(ref callbackVAC, (uint)block_size2, sample_rate2, host2, input_dev2, output_dev2, num_chan, 1, latency2);
			}
