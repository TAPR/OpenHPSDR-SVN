...
localparam SERIAL = 8'd17;  // software version serial number = 1.7
...
/*
	...
	<[90:89]DFS1,DFS0><[88]PTT><[87:84]address><[83:52]frequency><[51:49]clock_select><[48:42]OC>
...
begin
  ...
  begin
    ...
    C122_clock_select <= 3'b000;     
    OC                <= 7'b0;   // Penelope Open Collectors
	 Drive_Level 		 <= 8'b0;	// PennyLane drive level
  end
  else if (rcv_rdy)
  begin
    C122_DFS1    <= rcv_data[90];
    C122_DFS0    <= rcv_data[89];
    PTT_out      <= rcv_data[88];
    if (rcv_data[87:84] == PENNY_ADDR) // check that the C&C data is for this board
    begin
      C122_frequency_HZ <= rcv_data[83:52];
      C122_clock_select <= rcv_data[51:49];     
      OC                <= rcv_data[48:42]; 	// Penelope Open Collectors
		Drive_Level			<= rcv_data[15:8];		// PennyLane drive level	
    end
  end
end


NWire_rcv  #(.DATA_BITS(91), .ICLK_FREQ(122880000), .XCLK_FREQ(122880000), .SLOWEST_FREQ(10000)) 
