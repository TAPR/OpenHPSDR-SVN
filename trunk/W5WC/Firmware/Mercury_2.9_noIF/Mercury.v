...
localparam SERIAL = 8'd29;  // software version serial number - 2.9
...
begin
  ...
  begin
    ...
    begin
      C122_frequency_HZ <= C122_rcv_data[53:22];
...
NWire_rcv  #(.DATA_BITS(61), .ICLK_FREQ(122880000), .XCLK_FREQ(122880000), .SLOWEST_FREQ(5000)) 
...
//wire [31:0] C122_frequency_plus_IF; // add PowerSDR IF frequecy (9kHz) to current frequency

//assign C122_frequency_plus_IF = C122_frequency_HZ + 32'd9000; // add 9kHz IF offset 

LPF_select Alex_LPF_select(.clock(C122_clk), .frequency(C122_frequency_HZ), .LPF(C122_LPF));
HPF_select Alex_HPF_select(.clock(C122_clk), .frequency(C122_frequency_HZ), .HPF(C122_select_HPF));
...
assign C122_6m_preamp = (C122_frequency_HZ > 50000000) ? 1'b1 : 1'b0;
