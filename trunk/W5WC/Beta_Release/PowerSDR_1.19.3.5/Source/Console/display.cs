           ...
           if (bottom) g.FillRectangle(display_background_brush, 0, H, W, H);
            ...
            if (current_click_tune_mode != ClickTuneMode.Off)
            {
                ...
                else
                {
                    if (ClickTuneFilter)
                    {
                        double f_low = freq + (double)rx1_filter_low;
                        double f_high = freq + (double)rx1_filter_high;
                        int x1 = (int)Math.Round((f_low - low) / (high - low) * W);
                        int x2 = (int)Math.Round((f_high - low) / (high - low) * W);
		...
		unsafe static private bool DrawPanadapter(Graphics g, int W, int H, int rx, bool bottom)
		{
			...
			/* if(current_click_tune_mode != ClickTuneMode.Off)
			{
               ...
               // else p = Pens.Green;
				...
				{
                    double filter_low = freq + (double)rx1_filter_low;
                    double filter_high = freq + (double)rx1_filter_high;
                    int x1 = (int)Math.Round((filter_low - Low) / (High - Low) * W);
                    int x2 = (int)Math.Round((filter_high - Low) / (High - Low) * W);
                    g.FillRectangle(display_filter_brush, x1, 16, x2 - x1, H - 3);
