...
typedef struct _dp
{
	...
	OUTREAL *napixels[NUM_PIXEL_BUFFS];					//pointers to pixel output buffers for non-averaged data
	...
	volatile LONG na_pbready[NUM_PIXEL_BUFFS];
...
void GetNAPixels(	int disp,
					OUTREAL *pix,		//if new non-averaged pixel values avail, copies to pix and sets flag = 1
					int *flag			//else, returns 0 (try again later)
				);
