...
int rsize = -1;
void create_wisdom(int disp, char *app_data_path)
{
	DP a = pdisp[disp];
	fftw_plan tplan;
	int psize = 64;
	const char *wisdom_file = app_data_path;
	FILE *stream;
	...
	if(!fftw_import_wisdom_from_filename(wisdom_file))
	{
		...
		while (psize <= a->max_size)
		{
			rsize = psize;
			fprintf(stdout, "Planning FFT size %d\n", psize);
			fflush(stdout);
			tplan = fftw_plan_dft_r2c_1d(psize, a->fft_in[0][0], a->fft_out[0][0], FFTW_PATIENT);
			fftw_execute (tplan);
			fftw_destroy_plan (tplan);
			tplan = fftw_plan_dft_1d(psize, a->Cfft_in[0][0], a->fft_out[0][0], FFTW_FORWARD, FFTW_PATIENT);
			fftw_execute (tplan);
			fftw_destroy_plan (tplan);
			psize *= 2;
		}
		...
		//_tsystem(_T("taskkill /F /T /IM conhost.exe")); // close Console
	}
}

specHPSDR_PORT
int GetSpecWisdomStatus ( )
{
	return rsize;
}
...
void build_window(int disp, int type, int size, double PiAlpha)
{
	DP a = pdisp[disp];
	int i;
...
void stitch(int disp)
{
	...
	{			// for non-averaged data
		...
		for (n = a->begin_ss; n <= a->end_ss; n++)
			...
			{
				a->na_out[mna] = 10.0 * log10(a->scale * (a->result[n])[i] + 1.0e-60);
			}
	}
	...
	for (i = 0; i < a->num_pixels; i++)
		(a->napixels[a->na_wpixbuff])[i] = (OUTREAL)(a->na_tpixels[i] + 10.0 * log10(a->cd[i]));
	...
	case -1:	//peak detection
		{
			for (i = 0; i < a->num_pixels; i++)
			{
				...
				(a->pixels[a->w_pix_buff])[i] = (OUTREAL)(10.0 * log10(a->scale * a->cd[i] * a->av_sum[i] + 1e-60));
			}
			break;
		}
	case 0:		//no averaging
		{
			for (i = 0; i < a->num_pixels; i++)
				(a->pixels[a->w_pix_buff])[i] = (OUTREAL)(10.0 * log10(a->scale * a->cd[i] * a->t_pixels[i] + 1e-60));
			break;
		}
	...
	case 2:		//weighted averaging of log data
		{
			...
			for (i = 0; i < a->num_pixels; i++)
			{
				a->av_sum[i] = a->av_backmult * a->av_sum[i] + onem_avb * (10.0 * log10(a->scale * a->cd[i] * a->t_pixels[i] + 1e-60));
				(a->pixels[a->w_pix_buff])[i] = (OUTREAL)a->av_sum[i];
			}
			break;
		}
	case 3:		//window averaging of linear data
		{
			if (a->avail_frames < a->num_average)
			{
				...
				for (i = 0; i < a->num_pixels; i++)
				{
					...
					(a->pixels[a->w_pix_buff])[i] = (OUTREAL)(10.0 * log10(a->cd[i] * a->av_sum[i] * factor + 1e-60));
				}
			}
			else
			{
				...
				if (++a->av_out_idx == MAX_AVERAGE)
						a->av_out_idx = 0;
			}
			if (++a->av_in_idx == MAX_AVERAGE)
				a->av_in_idx = 0;
			break;
		}
	case 4:		//window averaging of log data
		{
			if (a->avail_frames < a->num_average)
			{
				...
				for (i = 0; i < a->num_pixels; i++)
				{
					(a->av_buff[a->av_in_idx])[i] = 10.0 * log10(a->cd[i] * a->t_pixels[i] * a->scale + 1e-60);
					a->av_sum[i] += (a->av_buff[a->av_in_idx])[i];
					(a->pixels[a->w_pix_buff])[i] = (OUTREAL)(a->av_sum[i] * factor);
				}
			}
			else
			{
	...
	EnterCriticalSection(&a->PB_ControlsSection);
		...
		while ((a->w_pix_buff = (a->w_pix_buff + 1) % NUM_PIXEL_BUFFS) == a->r_pix_buff);
	...
	EnterCriticalSection(&a->NA_PBControlsSection);
		...
		while ((a->na_wpixbuff = (a->na_wpixbuff + 1) % NUM_PIXEL_BUFFS) == a->na_rpixbuff);
...
int build_interpolants(int disp, int set, int n, int m, double *x, double (*y)[MAX_M])
{
	DP a = pdisp[disp];
	double dx[MAX_N];
	double idx[MAX_N];
	...
	double dsub[MAX_N];
	double dsup[MAX_N];
	double d[MAX_N][MAX_M];
	double S[MAX_N][MAX_M];
	double b[MAX_N];
	double v[MAX_N][MAX_M];
	double tmp;
	int i, j;
				 ...
				 )
{
	...
	if ((sz != a->size) || (win_type != a->window_type) || (pi != a->PiAlpha))
		build_window(disp, win_type, sz, pi);
	...
	for (i = 0; i < MAX_STITCH; i++)
		for (j = 0; j < MAX_NUM_FFT; j++)
			a->input_busy[i][j] = 0;
						...
						)
{
	...
	*success = 0;
	...
	a->pnum_threads = (LONG*) _aligned_malloc(4, 8);
	*(a->pnum_threads) = 0;
	...
	a->window = (double*) calloc(1, sizeof(double) * a->max_size);
	if (a->window == 0)
		*success = -1;
	for (i = 0; i < a->max_stitch; i++)
	{
		a->result[i] = (double*) calloc(1, sizeof(double) * a->max_size);
		if (a->result[i] == 0)
			*success = -1;
	}
	for (i = 0; i < a->max_stitch; i++)
		for (j = 0; j < a->max_num_fft; j++)
		{
			a->plan[i][j] = 0;
			a->Cplan[i][j] = 0;
			a->fft_in[i][j]   = (double*) calloc(1, sizeof(double) * a->max_size);
	...
	a->na_out     = (double*) calloc(1, sizeof(double) * a->max_size * a->max_stitch);
	...
	{
		a->av_buff[i] = (double*) calloc(1, sizeof(double) * MAX_PIXELS);
		if (a->av_buff[i] == 0)
			*success = -4;
	}
	...
	for (i = 0; i < NUM_PIXEL_BUFFS; i++)
	{
		a->pixels[i]   = (OUTREAL*) calloc(1, sizeof(OUTREAL) * MAX_PIXELS);
		a->napixels[i] = (OUTREAL*) calloc(1, sizeof(OUTREAL) * MAX_PIXELS);
		if (a->pixels[i] == 0 || a->napixels[i] == 0)
			*success = -5;
	}
	a->cd = (double*) calloc(1, sizeof(double) * MAX_PIXELS);
		if (a->cd == 0)
			*success = -6;
		else
	...
	{
		a->freqs[i] = (double*) calloc(1, sizeof(double) * MAX_N);
		for (j = 0; j < MAX_M; j++)
		{
			a->ac3[i][j] = (double*) calloc(1, sizeof(double) * MAX_N);
			a->ac2[i][j] = (double*) calloc(1, sizeof(double) * MAX_N);
			a->ac1[i][j] = (double*) calloc(1, sizeof(double) * MAX_N);
			a->ac0[i][j] = (double*) calloc(1, sizeof(double) * MAX_N);
			if((a->ac3[i][j] == 0) || (a->ac2[i][j] == 0) || (a->ac1[i][j] == 0) || (a->ac0[i][j] == 0))
				*success = -7;
		}
	}
	a->t_pixels   = (double*) calloc (1, sizeof(double) * MAX_PIXELS);
	a->na_tpixels = (double*) calloc (1, sizeof(double) * MAX_PIXELS);
	...
	a->bsize = a->max_size * SAMP_BUFF_MULT;
	for (i = 0; i < a->max_stitch; i++)
		for (j = 0; j < a->max_num_fft; j++)
		{
			a->I_samples[i][j] = (INREAL*) calloc(1, sizeof(INREAL) * a->bsize);
			a->Q_samples[i][j] = (INREAL*) calloc(1, sizeof(INREAL) * a->bsize);
			if((a->I_samples[i][j] == 0) || (a->Q_samples[i][j] == 0))
				*success = -9;
		}

	create_wisdom(disp, app_data_path);
}
...
{
	ICreateAnalyzer( disp, MAX_SIZE, MAX_NUM_FFT, MAX_STITCH, success, app_data_path );
}
...
{
	ICreateAnalyzer( disp, m_size, m_LO, m_stitch, success, app_data_path );
}
...
void DestroyAnalyzer(int disp)
{
	...
	for (i = 0; i < a->max_stitch; i++)
		for (j = 0; j < a->max_num_fft; j++)
		{
			if(a->I_samples[i][j]) free (a->I_samples[i][j]);
			if(a->Q_samples[i][j]) free (a->Q_samples[i][j]);
		}
	if (a->na_tpixels)			free (a->na_tpixels);
	if (a->t_pixels)			free (a->t_pixels);
	
	for (i = 0; i < MAX_CAL_SETS; i++)
	{
		if (a->freqs[i])		free (a->freqs[i]);
		for (j = 0; j < MAX_M; j++)
		{
			if (a->ac3[i][j])	free (a->ac3[i][j]);
			if (a->ac2[i][j])	free (a->ac2[i][j]);
			if (a->ac1[i][j])	free (a->ac1[i][j]);
			if (a->ac0[i][j])	free (a->ac0[i][j]);
		}
	}
	if (a->cd)					free (a->cd);
	for (i = 0; i < NUM_PIXEL_BUFFS; i++)
	{
		if (a->pixels[i])		free (a->pixels[i]);
		if (a->napixels[i])		free (a->napixels[i]);
	}
	for (i = 0; i < MAX_AVERAGE; i++)
		if (a->av_buff[i])		free (a->av_buff[i]);
	if (a->av_sum)				free (a->av_sum);
	if (a->pre_av_sum)			free (a->pre_av_sum);
	if (a->na_out)				free (a->na_out);
	...
	for (i = 0; i < a->max_stitch; i++)
		for (j = 0; j < a->max_num_fft; j++)
		{
			if (a->plan[i][j])			fftw_destroy_plan (a->plan[i][j]);
			if (a->Cplan[i][j])			fftw_destroy_plan (a->Cplan[i][j]);
			if (a->Cfft_in[i][j])		fftw_free (a->Cfft_in[i][j]);
			if (a->fft_in[i][j])		free (a->fft_in[i][j]);
			if (a->fft_out[i][j])		fftw_free (a->fft_out[i][j]);
		}
	if (a->window)				free (a->window);
	for (i = 0; i < a->max_stitch; i++)
		if (a->result[i])			free (a->result[i]);
	...
	if(a->pnum_threads) _aligned_free ((void *) a->pnum_threads);

	if(pdisp[disp])		free (pdisp[disp]);
}
					...
					)
{
	...
	{
		memcpy (pix, a->napixels[a->na_rpixbuff], a->num_pixels * sizeof(OUTREAL));
		*flag = 1;
													...
													//	 data for variables being calibrated
{
	...
	int i, j;
	int k = 0;
	double y [MAX_N][MAX_M];

	qsort (cal, n_points, (MAX_M+1) * sizeof(double), fcompare);
	...
	build_interpolants(disp, set_num, a->n_freqs[set_num], MAX_M, a->freqs[set_num], y);
	a->cal_changed = 1;
}
...
void Spectrum(int disp, int ss, int LO, INREAL* pI, INREAL* pQ)
{
	...
	memcpy(Ipointer, pI, a->buff_size * sizeof(INREAL));
	memcpy(Qpointer, pQ, a->buff_size * sizeof(INREAL));
...
specHPSDR_PORT   
void DebugData(int disp, void **Aptr, void **Bptr, void **Cptr, void **Dptr, void **Eptr, void **Fptr)
{
	DP a = pdisp[disp];
	*Aptr = &a->stitch_flag;
	*Bptr = &a->spec_flag;
