...
namespace SharpDSP
{
    ...
    unsafe public struct FFT_old
    {
        ...
        public FFT_old(int size)
        {
            ...
            fixed (double* ptr_w_cossintable = &this.w_cossintable[0])
            {
        ...
        public void DoComplexDFTForward(ref double[] real_in, ref double[] imag_in)
        {
            ...
            double* temp_buffer = stackalloc double[this.size * 2];
        ...
        private void cdft(int n, int isgn, double *a, int *ip, double *w)
        {                
		...
		private void cdft_fwd(int n, double *a, int *ip, double *w)
		{   
			bitrv2(n, ip + 2, a);
			cftfsub(n, a, w);			           
		}

		private void cdft_rev(int n, double *a, int *ip, double *w)
		{  
			bitrv2conj(n, ip + 2, a);
			cftbsub(n, a, w);            
		}

        private void rdft(int n, int isgn, double *a, int *ip, double *w)
        {
        ...
        private void makect(int nc, int *ip, double *c)
        {
            ...
            if (nc > 1) {
                nch = nc >> 1;
                delta = (double)(Math.Atan(1.0F) / nch);
                c[0] = (double)(Math.Cos(delta * nch));
                c[nch] = 0.5F * c[0];
                for (j = 1; j < nch; j++) {
                    c[j] = 0.5F * (double)(Math.Cos(delta * j));
                    c[nc - j] = 0.5F * (double)(Math.Sin(delta * j));
                }
            }
        }
