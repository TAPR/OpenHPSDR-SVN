...
{
	[Serializable()]
    unsafe public class FilterDesigner_old
    {       

        #region Private members

        private static double TWOPI = Convert.ToSingle(Math.PI * 2);
        private static double ONEPI = Convert.ToSingle(Math.PI);

        #endregion

        #region Public Methods

        public void MakeFirLowpass(double cutoff_freq, double samplerate, WindowType type, double* taps, int length)
        {
            double* window = stackalloc double[length];
            ...
            makewindow(type, length, window);

            for (int i = 1; i <= length; i++)
            {
                int j = i - 1;
                if (i != midpoint)
                    taps[j] = (Convert.ToSingle(Math.Sin(TWOPI * (i - midpoint) * fc) / (ONEPI * (i - midpoint)))) * window[j];
                else
        ...
        public void MakeFirBandpass(double lo, double hi, double samplerate, WindowType type, double* taps_real, double* taps_imag, int length)
        {
            double* window = stackalloc double[length];
            ...
            for (int i = 1; i <= length; i++)
            {
                ...
                if (i != midpoint)
                    temp = ((double)(Math.Sin(TWOPI * k * fc) / (ONEPI * k))) * window[j];
                else
                    temp = 2.0F * fc;
                temp *= 2.0F;
                taps_real[j] = temp * (double)(Math.Cos(phase));
                taps_imag[j] = temp * (double)(Math.Sin(phase));
            }

        }

        public void makewindow(WindowType type, int size, double *window) 
        {
            ...
            midp1 = (size + 1) / 2;
            midm1 = (size - 1) / 2;
            two_pi = 8.0F * (double)Math.Atan(1.0F);
            freq = two_pi / (double) size;
            rate = 1.0F / (double) midn;
            angle = 0.0F;
            expn = (double)Math.Log(2.0F) / (double) midn + 1.0F;
            expsum = 1.0F;
            ...
            switch (type) 
            {
                ...
                case WindowType.HANNING_WINDOW:	/* Hann would be more accurate */
                    for (i = 0, j = size - 1, angle = 0.0F; i <= midn; i++, j--, angle += freq)
						window[j] = (window[i] = 0.5F - 0.5F * (double)Math.Cos(angle));
                    break;
                case WindowType.WELCH_WINDOW:
                    for (i = 0, j = size - 1; i <= midn; i++, j--)
						window[j] = (window[i] = 1.0F - (double)Math.Sqrt((double) (i - midm1) / (double) midp1));
                    break;
                ...
                case WindowType.RIEMANN_WINDOW:
	                ...
	                cx = sr1 * (midn - i);
	                window[i] = (double)Math.Sin(cx) / cx;
	                window[j] = window[i];
                    }
                    }
                    break;
                case WindowType.BLACKMANHARRIS_WINDOW:
                    {
		                ...
		                for (i = 0; i<size;i++) 
                        {
			                window[i] = a0 - a1* (double)Math.Cos(two_pi*(double)(i+0.5)/(double)size) 
				                + a2* (double)Math.Cos(2.0*two_pi*(double)(i+0.5)/(double)size)
				                - a3* (double)Math.Cos(3.0*two_pi*(double)(i+0.5)/(double)size);
		                }
	                }
                    break;
                default:
