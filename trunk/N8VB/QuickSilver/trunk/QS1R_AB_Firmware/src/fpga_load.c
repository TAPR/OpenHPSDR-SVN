...
fpga_load_begin (void)
{
	...
	QS1R_ALTERA_CONFIG &= ~bmALTERA_BITS;		// clear all bits (NCONFIG low)
  	udelay (40);					// wait 40 us
  	QS1R_ALTERA_CONFIG |= bmALTERA_NCONFIG;	// set NCONFIG high

  	while ((QS1R_ALTERA_CONFIG & bmALTERA_NSTATUS) == 0)
    {
        counter++;
        udelay(50);
        if (counter  >= 255)
...
{
    char the_bits;          // why is bits passed? 12/07/2006 PAC
    the_bits = bits;        // this is so we don't get a warning for
                                // not using bits
...
fpga_load_end (void)
{
  unsigned char status = QS1R_ALTERA_CONFIG;
