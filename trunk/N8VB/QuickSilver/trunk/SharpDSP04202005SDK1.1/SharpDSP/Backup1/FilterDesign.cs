...
{
	[Serializable()]
    unsafe public class FilterDesigner
    {       
        ...
        public void MakeFirLowpass(float cutoff_freq, float samplerate, WindowType type, float* taps, int length)
        {
            float* window = stackalloc float[length];
        ...
        public void MakeFirBandpass(float lo, float hi, float samplerate, WindowType type, float* taps_real, float* taps_imag, int length)
        {
            float* window = stackalloc float[length];
            ...
            for (int i = 1; i <= length; i++)
            {
                ...
                if (i != midpoint)
                    ...
                    temp = 2.0F * fc;
                temp *= 2.0F;
                taps_real[j] = temp * (float)(Math.Cos(phase));
                taps_imag[j] = temp * (float)(Math.Sin(phase));
            }

        }

        public void makewindow(WindowType type, int size, float *window) 
        {
