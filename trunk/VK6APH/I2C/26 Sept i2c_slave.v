...
always @(negedge scl_in or negedge rst_n)
  if(~rst_n)
    ...
    sda_oe <= target & (i2c_sm == `SM_READ) & ~cnt7 |
              (i2c_sm == `SM_ADDR) & cnt7 & addr_dec |
              (i2c_sm == `SM_WRITE) & cnt7;

// wire sda_out = ~((i2c_sm == `SM_WRITE) & cnt8);     
wire  sda_out = 1'b0;
...
assign debug = {stop,start,rcv_cntr,sda_oe,i2c_sm};
