...
reset_OvSv(FiltOvSv pflt) {
  ...
  memset((char *) pflt->zovec, 0, pflt->fftlen * sizeof(COMPLEX));		// #### was in Melton's version
}
...
newFiltOvSv(COMPLEX * coefs, int ncoef, int pbits) {
  ...
  normalize_vec_COMPLEX (p->zfvec, p->fftlen, p->scale);	// #### was in Melton's version
  return p;
}
