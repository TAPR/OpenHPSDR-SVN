...
typedef enum _sdrmode			// is this correct for SetMode() ???
{
  LSB,				//  0
  USB,				//  1
  DSB,				//  2
...
extern int SetMode(uint thread, uint subrx, DSPMode m);
extern int SetRXFilter(uint thread, uint subrx, double low, double high);
extern void SetRXAGC(uint thread, uint subrx, AGCMode setit);
extern void SetNR(uint thread, uint subrx, bool setit);
extern void SetNB(uint thread, uint subrx, bool setit);
extern void SetANF(uint thread, uint subrx, bool setit);
extern void SetTRX(uint thread, bool trx_on);
extern void SetThreadProcessingMode(uint thread, int runmode);
extern void SetSubRXSt(uint thread, uint subrx, bool setit);
