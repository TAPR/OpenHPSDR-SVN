 ...
 * Portions Copyright (C) 2010 Bill Tracey, KD5TFD 
 *
    ...
    public class libUSB_Interface
    {
        ...
        */
        ...
        public class usb_bus 
        {
            ...
            public void dump()
            {
                Console.WriteLine("dump of usb bus object"); 
                ...
                {
                    Console.WriteLine("devices: " + devices.ToInt32().ToString("X8"));
                }                    
            } 
            ...
            public usb_device GetDevices()
            {
                if (devices == IntPtr.Zero)
                {
                    Console.WriteLine("GetDevices returning null!");
                    return null;
                }
                else
                {
                    Console.WriteLine("GetDevices returning non null!");
        ...
        [DllImport(LIBUSB_NATIVE_LIBRARY, CallingConvention = CALLING_CONVENTION, EntryPoint="usb_control_msg", ExactSpelling = true), SuppressUnmanagedCodeSecurity]
        public static extern int usb_control_msg_native(IntPtr dev, int requesttype, int request, int value, int index, byte[] bytes, int size, int timeout);

        public static int usb_control_msg(IntPtr dev, int requesttype, int request, int value, int index, byte[] bytes, int size, int timeout)
        {
            if (dev.Equals(IntPtr.Zero))
            {
                Console.WriteLine("usb_control_msg: dev is null!");
                return 0;
            }
            return usb_control_msg_native(dev, requesttype, request, value, index, bytes, size, timeout);            
        }
        ...
        [DllImport(LIBUSB_NATIVE_LIBRARY, CallingConvention = CALLING_CONVENTION, EntryPoint="usb_get_busses", ExactSpelling = true), SuppressUnmanagedCodeSecurity]
        public static extern IntPtr usb_get_busses_native();

        // private static usb_bus global_bus = null; 

        public static usb_bus usb_get_busses()
        {
            //if (global_bus != null)
            //{
            //    // Console.WriteLine("returning global bus"); 
            //    return global_bus; 
            //}

            

            IntPtr p  = usb_get_busses_native();
            // Console.WriteLine("usb_get_busses_native returned: " + p.ToInt32().ToString("X8")); 
            //unsafe
            //{
            //    int *ip = (int *)(p.ToPointer()); 
            //    ip += 130;
            //    int i = *ip;
            //    Console.WriteLine("ip addr: " + ((int)(ip)).ToString("X8"));
            //    Console.WriteLine("devices addr: " + i.ToString("X8"));
            //}

            if (p != IntPtr.Zero)
            {

                // global_bus = (usb_bus)Marshal.PtrToStructure(p, typeof(usb_bus));
                // Console.WriteLine("setting global bus"); 
                // global_bus.dump();
                // return global_bus;
                return (usb_bus)Marshal.PtrToStructure(p, typeof(usb_bus));
            }
            else
            {
                return null;
