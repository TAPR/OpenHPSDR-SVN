...
)
{
	...
	char status[1024];
	jstring utf8;
        __android_log_write(ANDROID_LOG_ERROR, "WDSPWisdom", "entry");
        jclass cls = (*env)->GetObjectClass(env, obj);
        jmethodID update = (*env)->GetMethodID(env, cls, "updateStatus", "(Ljava/lang/String;)V");
	sprintf(status,"updateStatus id=%d",(int)update);
        __android_log_write(ANDROID_LOG_ERROR, "WDSPWisdom", status);
#endif
