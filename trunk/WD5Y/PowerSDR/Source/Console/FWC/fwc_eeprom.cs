...
namespace PowerSDR
{
	public class FWCEEPROM
	{
		...
		public static bool NeedDump()
		{
			...
			StringBuilder s = new StringBuilder(((byte)(data)).ToString("00"));
			s.Append(((byte)(data>>8)).ToString("00"));
			s.Append("-"+((ushort)(data>>16)).ToString("0000"));
			if(File.Exists(Application.StartupPath+"\\"+s+" backup.csv"))
				return false;
			return true;
		}
		...
		private static void Dump()
		{
			...
			StreamWriter writer = new StreamWriter(Application.StartupPath+"\\"+s+" backup.csv");
		...
		private static Model model = Model.FLEX5000;
		...
		public static void WriteRXImage(float[] gain_table, float[] phase_table, out byte gain_sum, out byte phase_sum)
		{
			...
			for(uint i=0; i<bands.Length; i++)
			{
				if(gain_table[(int)bands[i]] > 500.0f || gain_table[(int)bands[i]] < -500.0f)
				{
					...
					TextWriter writer = new StreamWriter(Application.StartupPath+"\\eeprom_error.log", true);
		...
		public static void ReadRXImage(float[] gain_table, float[] phase_table)
		{
			...
			for(uint i=0; i<bands.Length; i++)
			{
				...
				gain_table[(int)bands[i]] = (float)Math.Round(f, 2);
				...
				phase_table[(int)bands[i]] = (float)Math.Round(f, 2);
			}
		}
