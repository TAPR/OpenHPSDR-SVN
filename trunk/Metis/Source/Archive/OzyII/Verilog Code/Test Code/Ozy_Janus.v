...
//			   26 June 2009 - ******  test version for USB  *******
//							- fix bug where if we have data to send and EP6 not ready we loose it 
//					        - read all EP2 data at once rather that just one sample
//			   27 June 2009 - combined fifo level and EP6   e.g. (syncd_write_used > 511 && EP6_ready)
//							- Rx and Tx FIFO = 4096
//							- Time out 1mS at 48MHz  can make much larger for PowerSDR but not v1.0.1 KK 
//							- Grab all available data from EP2
//							- send 511 words to EP6
//							- Works OK with PowerSDR but KK V1.0.1 fails at 48k and glitches at 192k
//							- EP4 OK, reads one sample at a time
//				4 July 2009 - Can extend EP6 time out to seconds for PSDR but not KK - OK at 96kHz, in fact better
//							  but not 192k. Replaced full = !EP4_ready with full = write_full and also check 
//							  write_full for spectum FIFO before writing to it.
//							- Current code reads on word from EP1 and then writes one word the EP6 and EP4
//							- Used seperate Megafunction for spec FIFO
//							- Can make EP2 FIFO 2048 and EP6 FIFO 4096 for PowerSDR but not KK so leave as is for now.
//							- next mod code to read 4 words from EP4 then send 4 words to EP6 and EP4 with SLWR etc set for each.
//
////////////////////////////////////////////////////////////
...
assign full = write_full; // !EP4_ready; // stop sending data if FIFO full
...
// Check that EP6 is available - if not available for > ~1mS then set halt flag
...
begin
	if (EP6_ready)begin
		...
		if (halt_count > 50000)  halt_flag <= 1'b1; // was 50,000 > OK for PowerSDR but not KK
...
spec_fifo FIFO_2(.wrclk (!BCLK),.rdreq (Tx_read_clock_2),.rdclk (IFCLK),.wrreq(Tx_fifo_enable_2),
                .data (EP4_data),.q (Tx_register_2), .wrusedw(write_used_2), .wrfull (write_full), .rdempty(Tx_empty_2));
...
// get next word without raising SLRD        
5:  begin
	read_data <= 1'b1;    // enable sync decoder, previous data   
...
7:begin
        if (EP6_ready && !Tx_empty) begin // data available, so let's start the xfer...
			...
			FIFO_ADR <= 2'b01;
			state_FX <= 11;                  // No EP6 data so check EP4 data,
			end
    end                                     
8:begin
    	Tx_read_clock <= 1'b0;          	   
...
10: begin
        SLWR <= 1;
        SLEN <= 1'b0;
        //if (send_count == 511)				// send 511 words 
			state_FX <= state_FX + 1'b1;
		//else begin
...
11: // delay for address to be stable
		state_FX <= state_FX + 1'b1;
		
		
// Send data to EP4 
12:begin
	if (EP4_ready && !Tx_empty_2) begin                        // if EP4 is ready, write to it and exit this state
...
1:
  begin
    if (LRCLK) spec  <= 1;     		// loop until LRCLK is low 
		...
		if (write_full) spec <= 0;  // skip this time and try the next
		else spec  <= 2;
	end 
  end
