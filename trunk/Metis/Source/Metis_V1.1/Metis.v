...
/*  4 March 2011 - Metis code V1.1 - copyright 2010, 2011 Phil Harman VK6APH 
	...
	NOTES:  	  	- Under Settings > Analysis & Synthesis Settings > Verilig HDL Input, select SystemVerilog-2005 so can pass RAM to a module
					- Spectrum data not presently sent - Atlas C21 not connected
...
output	wire	 ATLAS_C21; // 				Spectrum data Trigger signal to Mercury
...
/*  For some reason Atlas buss lines C25-C31 do not work for me between Mercury and Metis...using
    A4 and A6 instead for now for dual Mercury board operations...K5SO
    
    // multiple Mercury firmware version and ADC overload input lines
    input   wire     ATLAS_C25; // Merc4 _version & ADC overload from Mercury 4 (Multiple Rx-3)
    input   wire     ATLAS_C27; // Merc3 _version & ADC overload from Mercury 3 (Multiple Rx-2)
    input   wire     ATLAS_C29; // Merc2 _version & ADC overload from Mercury 2 (Multiple Rx-1)
    //Note:  ATLAS_C31 is not used for Mercury 1 version /overload signals because historically
    //ATLAS_A6 is used for Mercury 1 (Multiple Rx-0) for that purpose, thus ATLAS_A6 is used in
    //this firmware version instead of the more logical ATLAS_31 in order to maintain compatibility
    //with older versions of Mercury firmware
*/
...
localparam Metis_serialno = 8'd11;	// Serial number of this version
//localparam NR = 1;                  //Maximum number of Mercury receiver channels
...
Rx_MAC Rx_MAC_inst (.PHY_RX_CLOCK(PHY_RX_CLOCK), .PHY_data_clock(PHY_data_clock),.PHY_DV(PHY_DV), .PHY_RX(PHY_RX),
			        ...
			        .METIS_discovery(METIS_discovery), .PC_IP(PC_IP), .PC_MAC(PC_MAC), .This_IP(This_IP), 
			        .Length(Length), .PHY_100T_state(PHY_100T_state),
...
NWire_rcv  #(.DATA_BITS(9), .ICLK_FREQ(48000000), .XCLK_FREQ(48000000), .SLOWEST_FREQ(500)) 
      m_ver3 (.irst(IF_rst), .iclk(IF_clk), .xrst(IF_rst), .xclk(IF_clk),
             .xrcv_data(IF_md3), .xrcv_rdy(IF_md_rdy3), .xrcv_ack(IF_md_rdy3), .din(ATLAS_C27));
             
NWire_rcv  #(.DATA_BITS(9), .ICLK_FREQ(48000000), .XCLK_FREQ(48000000), .SLOWEST_FREQ(500)) 
      m_ver3 (.irst(IF_rst), .iclk(IF_clk), .xrst(IF_rst), .xclk(IF_clk),
             .xrcv_data(IF_md4), .xrcv_rdy(IF_md_rdy4), .xrcv_ack(IF_md_rdy4), .din(ATLAS_C25));
*/             
