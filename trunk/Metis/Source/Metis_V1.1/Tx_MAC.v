...
module Tx_MAC (Tx_clock, Tx_clock_2, IF_rst, Send_ARP,ping_reply,
			   ...
			   Ping_PC_MAC, Ping_PC_IP, Length, speed_100T, Tx_reset, run,
			   IP_valid, printf, IP_lease, DHCP_IP, DHCP_MAC, DHCP_request_renew, DHCP_request_renew_sent,
			   erase_done, erase_done_ACK, send_more, send_more_ACK);
...
// Start of Payload
  ...
  54: pkt_data <= sequence_number[31:24]; // 32bit sequence number 
  55: pkt_data <= sequence_number[23:16];
  56: pkt_data <= sequence_number[15:8];
  57: pkt_data <= sequence_number[7:0];  
...
// Ethernet preamble
 ...
 507: pkt_data <= 8'hD5;
// Ethernet header
 508: pkt_data <= PC_MAC[47:40];
 509: pkt_data <= PC_MAC[39:32];
 510: pkt_data <= PC_MAC[31:24];
 511: pkt_data <= PC_MAC[23:16];
 512: pkt_data <= PC_MAC[15:8];
 513: pkt_data <= PC_MAC[7:0];
 ...
 538: pkt_data <= PC_IP[31:24];			// Destination PC IP Address
 539: pkt_data <= PC_IP[23:16];
 540: pkt_data <= PC_IP[15:8];
 541: pkt_data <= PC_IP[7:0];
// UDP header
 542: pkt_data <= TxPort[15:8];			// 'from' port
 543: pkt_data <= TxPort[7:0];
 544: pkt_data <= Port[15:8];			// destination port assigned by PC
 545: pkt_data <= Port[7:0];
...
// Start of Payload
 ...
 551: pkt_data <= Type_2;
 552: pkt_data <= frame;				// HPSDR Frame type = discovery reply = 0x02
 ...
 // send 51 zeros to give a minimum payload of 60 bytes
 // then CRC 
...
assign DHCP_req_length = 16'd8 + (16'd473 - 16'd450 + 16'd1) + 16'd24 + 16'd202; // 8 + 250
...
METIS_DISCOVERY:
	begin
		if (rdaddress != 559) begin						// keep sending until we reach the end of the data 
			Tx_data <= pkt_data;
			sync_Tx_CTL <= 1'b1;					// enable write to PHY
			rdaddress <= rdaddress + 1'b1;
			state_Tx <= METIS_DISCOVERY;
		end
		else if (zero_count < 51)begin				// send 51 x 0x00s
			Tx_data <= 8'h00;
...
DHCP_REQUEST:
	begin
		...
		else if (rdaddress < 474) begin				// send the balance of the data				
				Tx_data <= pkt_data;
				rdaddress <= rdaddress + 1'b1;
				state_Tx <= DHCP_REQUEST;
		end
