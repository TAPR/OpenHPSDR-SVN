...
/* 
	...
	2011 Mar  11    - Send DHCP server IP address as an option in Request to try and fix Dan and Joe's issues.
	     Apr   2    - Reset sequence number when run goes from 0 to 1. This is so we can run  
	                  multiple Metis boards on the same network. 
...
reg previous_run;					// so we can detect when run changes state
...
// Start of Payload
 ...
 551: pkt_data <= Type_2;
 552: pkt_data <= frame + run;			// HPSDR Frame type = discovery reply = 0x02 or 0x02 if running
   ...
   begin
	...
	send_more_ACK <= 0;
        ...
        if (IF_rst)
			...
			if (run == 1'b1 && previous_run == 0)	// detect start command and reset sequence number
				sequence_number <= 0;
			previous_run <= run;
			if (printf) begin
				rdaddress <= 600;
				...
				METIS_discover_sent <= 1'b1;				// let Rx know Discovery has been responded to
