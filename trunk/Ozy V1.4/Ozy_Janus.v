//  V3 10 June, 2009 by Kirk Weedman KD7IRS (V1.4)
//
...
localparam Ozy_serialno = 8'd14;	// Serial number of this version
...
begin
  ...
  if (IF_rst)
    ...
    IF_xmit_req    <= #IF_TPD (IF_PWM_state == PWM_WAIT) ? 1'b1 : 1'b0; // all data ready to be sent now
...
//      IF_PWM_state_next  = PWM_IDLE;
    ...
    PWM_WAIT: // got donuts, now wait for customer to buy donuts or donuts get old and we toss them
    begin
      Rx_fifo_rreq       = 1'b0;
      if (IF_get_rx_data)
        IF_PWM_state_next  = PWM_IDLE;
...
begin: SAMPLES
  if (IF_rst)
    ...
    m <= {m[1:0], C23}; // C23 = M_LR_sync

  if (IF_rst)
    p <= 3'b000;
  else 
    p <= {p[1:0], C22}; // C22 = P_IQ_sync
...
begin
  if (conf[1])
    IF_get_samples = !m[1] & m[2];  // Mercury installed so use falling edge of C23 (M_LR_sync)
  else if (conf[0])
    IF_get_samples = !p[1] & p[2];  // Penelope installed so use falling edge of C22 (P_IQ_sync)
  else
    IF_get_samples = !j[1] & j[2];  // use Janus so use falling edge of C23 (C12_CLRCLK)
end
...
I2S_xmit #(.DATA_BITS(32))
  J_LRAudio (.rst(C12_rst), .lrclk(C12_CLRCLK), .clk(C12_CBCLK), .CBrise(C12_cbrise),
             ...
             .xmit_rdy(C12_j_rdy), .xmit_ack(C12_j_rdy));
