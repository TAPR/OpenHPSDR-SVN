...
// V2.7 18 May 2009 Copyright 2009 Kirk Weedman KD7IRS
...
localparam SERIAL = 8'd27;  // software version serial number - 2.7
...
localparam MERCURY_ADDR = 4'b0; // Address for data
...
begin
  ...
  begin
    ...
    C122_DFS0     <= rcv_data[59];    // I/Q sampling rate selection
    C122_PTT_out  <= rcv_data[58];    // PTT from PC via USB 
    if (rcv_data[57:54] == MERCURY_ADDR) // check that the C&C data is for this board
    begin
      ...
      C122_ATTEN        <= rcv_data[6:5];   // Attenuator setting on Alex
      C122_TX_relay     <= rcv_data[4:3];   // Tx relay selection on Alex
      C122_Rout         <= rcv_data[2];     // Rx_1_out on Alex
      C122_RX_relay     <= rcv_data[1:0];   // Rx relay selection on Alex
    end
  end
  else
    C122_new_data <= 1'b0;
...
assign {DEBUG_LED2,DEBUG_LED1} = counter[26:25];  // Slower  flash for this version!
