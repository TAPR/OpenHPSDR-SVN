...
*/
		...
		private static void DrawPanadapterGrid(ref Graphics g, int W, int H)
		{
			...
			{
				double band_edge_offset = band_edge_list[i] - vfoa_hz;
		...
		private static void DrawWaterfallGrid(ref Graphics g, int W, int H)
		{
			...
			if(!mox)					// get left and right drawing limits
			{
				low = rx_display_low;
				high = rx_display_high;
			}
			...
			if(high_swr)
				g.DrawString("High SWR", new System.Drawing.Font("Arial", 12, FontStyle.Bold), new SolidBrush(Color.Red), 245, 0);
		}
		...
		unsafe static private bool DrawWaterfall(Graphics g, int W, int H)
		{
			...
			for(int i=0; i<W; i++)
			{
				float max = float.MinValue;
				float dval = i*slope + start_sample_index;
				int lindex = (int)Math.Floor(dval)%4096;
				if (slope <= 1) 
					max =  current_display_data[lindex]*((float)lindex-dval+1) + current_display_data[(lindex+1)%4096]*(dval-(float)lindex);
				else 
				{
					int rindex = (int)Math.Floor(dval + slope);
					if (rindex > BUFFER_SIZE) rindex = BUFFER_SIZE;
