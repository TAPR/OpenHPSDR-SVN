...
#define FPGA_INPUT_FIFO_SIZE (2048) 
...
void swapBytesShort(short *sp, int count) { 
	unsigned char *p; 
	unsigned char tmp; 
	int i; 
	for ( i = 0; i < count; i++ ) { 
...
void IOThreadMainLoop(void) { 
	...
	sample_bufp_size = 2*sizeof(int)*BlockSize; 	
	outbuflen = 2 * sizeof(short) * BlockSize; 
...
typedef struct zperf_record { 
		...
		if ( PerfDataIdx > 0 ) { 
			...
			for ( i = 0; i < PerfDataIdx; i++ ) { 
				...
				printf("i: %d fa: %d fp: %d fs: %d or: %d w: %d okw: %d rt: %I64d wt: %I64d drt: %I64d dwt: %I64d cb: %d t: %I64d dt: %I64d\n",  i, 
