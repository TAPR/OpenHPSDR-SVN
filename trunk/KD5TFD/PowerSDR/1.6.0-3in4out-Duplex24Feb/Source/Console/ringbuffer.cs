	...
	unsafe public class RingBuffer
	{
		...
		public int Read(float[] dest, int cnt)
		{
			...
			fixed(void *r = &buf[rptr])
				fixed(void *w = &dest[0])
					MSCVRT.memcpy(w, r, n1*sizeof(float));
			//MSCVRT.memcpy(dest, buf[rptr], n1);
//			for(int i=0; i<n1; i++)
//				dest[i] = buf[rptr+i];
			...
			if(n2 != 0) 
			{
				fixed(void *r = &buf[rptr])
					fixed(void *w = &dest[n1])
						MSCVRT.memcpy(w, r, n2*sizeof(float));
				//MSCVRT.memcpy(dest + n1, buf[rptr], n2);
//				for(int i=0; i<n2; i++)
...
//				buf[wptr+i] = src[i];
			...
			if(n2 != 0) 
			{
				fixed(void *r = &src[n1])
					fixed(void *w = &buf[wptr])
						MSCVRT.memcpy(w, r, n1*sizeof(float));
				//MSCVRT.memcpy(buf[wptr], src + n1, n2);
				for(int i=0; i<n2; i++)
					buf[wptr+i] = src[i+n1];
