...
localparam SERIAL = 8'd15;  // software version serial number = 1.5
...
begin
  ...
  begin
    C122_DFS1    <= rcv_data[83];
    C122_DFS0    <= rcv_data[82];
    PTT_out      <= rcv_data[81];
    if (rcv_data[80:77] == PENNY_ADDR) // check that the C&C data is for this board
    begin
      C122_frequency_HZ <= rcv_data[76:45];
      C122_clock_select <= rcv_data[44:41];     
      OC                <= rcv_data[40:34]; 	// Penelope Open Collectors
		Drive_Level			<= rcv_data[7:0];		// PennyLane drive level	
    end
  end
end


NWire_rcv  #(.DATA_BITS(84), .ICLK_FREQ(122880000), .XCLK_FREQ(122880000), .SLOWEST_FREQ(500)) 
