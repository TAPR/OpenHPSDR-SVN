...
/* 
   ...
   2012 Dec   29     - Added support for read/write IP address when not in bootloader mode
   ...
   begin
	...
	IP_count <= 0;		
        ...
        if (IF_rst)
   		...
   		else if (have_sp_data && wide_spectrum) begin	// Wide Spectrum enabled and fifo has data so send it
				rdaddress <= 0;																	
				state_Tx <= SPECTRUM;
			end
			else	state_Tx <= RESET;
