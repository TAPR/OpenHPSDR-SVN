...
/**
 * Launch a thread to listen for TCP connection requests
 */
...
/**
 * Open a TCP socket on which to receive connection request.
 * \param arg A void pointer to nothing--not used
 */
void* listener_thread(void* arg) {
    int s;
    int address_length;
...
fprintf(stderr,"Listening for TCP connections on port %d\n",LISTEN_PORT);
    ...
    while(1) {
		...
		// Build a struct client that we'll pass to the client_thread about to be launched.
